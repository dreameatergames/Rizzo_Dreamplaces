...
static void SV_SaveEntFile_f(void)
{
	...
	FS_WriteFile(va("%s.ent", basename), sv.worldmodel->t.brush.entities, (fs_offset_t)strlen(sv.worldmodel->t.brush.entities));
}
...
qboolean SV_PrepareEntityForSending (prvm_edict_t *ent, entity_state_t *cs, int e)
{
	...
	*cs = defaultstate;
	...
	if (!VectorCompare(cullmins, ent->priv.server->cullmins) || !VectorCompare(cullmaxs, ent->priv.server->cullmaxs))
	{
		...
		if (sv.worldmodel && sv.worldmodel->t.brush.FindBoxClusters)
		{
			i = sv.worldmodel->t.brush.FindBoxClusters(sv.worldmodel, cullmins, cullmaxs, MAX_ENTITYCLUSTERS, ent->priv.server->pvs_clusterlist);
		...
		else if (!(s->effects & EF_NODEPTHTEST) && (!isbmodel || !sv_cullentities_nevercullbmodels.integer || sv.protocol == PROTOCOL_QUAKE || sv.protocol == PROTOCOL_QUAKEDP || sv.protocol == PROTOCOL_NEHAHRAMOVIE))
		{
			...
			if (sv_cullentities_pvs.integer && sv_writeentitiestoclient_pvsbytes)
			{
				if (ed->priv.server->pvs_numclusters < 0)
				{
					...
					if (sv.worldmodel && sv.worldmodel->t.brush.BoxTouchingPVS && !sv.worldmodel->t.brush.BoxTouchingPVS(sv.worldmodel, sv_writeentitiestoclient_pvs, ed->priv.server->cullmins, ed->priv.server->cullmaxs))
					{
			...
			if (sv_cullentities_trace.integer && !isbmodel)
			{
				...
				do
				{
					...
					if(sv_cullentities_trace_prediction.integer)
					{
						...
						if(sv.worldmodel->t.brush.TraceLineOfSight(sv.worldmodel, sv_writeentitiestoclient_testeye, predeye)) // must be able to go there...
						{
...
// find the client's PVS
	...
	if (sv.worldmodel && sv.worldmodel->t.brush.FatPVS)
		sv_writeentitiestoclient_pvsbytes = sv.worldmodel->t.brush.FatPVS(sv.worldmodel, sv_writeentitiestoclient_testeye, 8, sv_writeentitiestoclient_pvs, sizeof(sv_writeentitiestoclient_pvs));
...
//
	...
	for (i = 1;i < sv.worldmodel->t.brush.numsubmodels;i++)
	{
...
// serverflags are for cross level information (sigils)
	...
	if (sv_entpatch.integer && (entities = (char *)FS_LoadFile(va("maps/%s.ent", sv.name), tempmempool, true, NULL)))
	{
		...
		PRVM_ED_LoadFromFile (sv.worldmodel->t.brush.entities);
...
#else
	prog->error_cmd = (void (*)(const char*, ...))&Host_ShutdownServer;
#endif
...
{
	dreamplaces_prof_start(PROFILER_SV);
	PRVM_Begin;
...
{
	PRVM_End;
	dreamplaces_prof_end(PROFILER_SV);
}
