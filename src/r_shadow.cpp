...
{
	float dist = dr_sqrt(x*x+y*y+z*z);
...
static void R_Shadow_RenderLighting_Light_Vertex_Shading(int firstvertex, int numverts, int numtriangles, const int *element3i, const float *diffusecolor, const float *ambientcolor)
{
	...
	if (r_textureunits.integer >= 3)
	{
		if (VectorLength2(diffusecolor) > 0)
		{
			for (;numverts > 0;numverts--, vertex3f += 3, normal3f += 3, color4f += 4)
			{
				...
				{
					shadeintensity = -dot / dr_sqrt(VectorLength2(v) * VectorLength2(n));
	...
	else if (r_textureunits.integer >= 2)
	{
		if (VectorLength2(diffusecolor) > 0)
		{
			...
			{
				Matrix4x4_Transform(&r_shadow_entitytolight, vertex3f, v);
				if ((dist = dr_fabs(v[2])) < 1 && (distintensity = r_shadow_attentable[(int)(dist * ATTENTABLESIZE)]))
				{
					Matrix4x4_Transform3x3(&r_shadow_entitytolight, normal3f, n);
					if ((dot = DotProduct(n, v)) < 0)
					{
...
static void R_Shadow_GenTexCoords_Diffuse_NormalCubeMap(int firstvertex, int numvertices, int numtriangles, const int *element3i)
{
	...
	float lightdir[3];
	for (i = 0;i < numvertices;i++, vertex3f += 3,
#ifndef NO_ST_VECTORS
		       	svector3f += 3, tvector3f += 3,
#endif
		       	normal3f += 3, out3f += 3)
	{
...
void R_RTLight_Compile(rtlight_t *rtlight)
{
	...
	if (model && model->GetLightInfo)
	{
		...
		R_Shadow_EnlargeLeafSurfaceTrisBuffer(model->t.brush.num_leafs, model->num_surfaces, model->t.brush.shadowmesh ? model->t.brush.shadowmesh->numtriangles : model->surfmesh.num_triangles, model->surfmesh.num_triangles);
		...
		numleafpvsbytes = (model->t.brush.num_leafs + 7) >> 3;
		numshadowtrispvsbytes = ((model->t.brush.shadowmesh ? model->t.brush.shadowmesh->numtriangles : model->surfmesh.num_triangles) + 7) >> 3;
...
void R_Shadow_DrawWorldShadow(int numsurfaces, int *surfacelist, const unsigned char *trispvs)
{
	...
	else if (numsurfaces && r_refdef.worldmodel->t.brush.shadowmesh && r_shadow_culltriangles.integer)
	{
		int t, tend;
		int surfacelistindex;
		msurface_t *surface;
		R_Shadow_PrepareShadowMark(r_refdef.worldmodel->t.brush.shadowmesh->numtriangles);
		...
		R_Shadow_VolumeFromList(r_refdef.worldmodel->t.brush.shadowmesh->numverts, r_refdef.worldmodel->t.brush.shadowmesh->numtriangles, r_refdef.worldmodel->t.brush.shadowmesh->vertex3f, r_refdef.worldmodel->t.brush.shadowmesh->element3i, r_refdef.worldmodel->t.brush.shadowmesh->neighbor3i, r_shadow_rtlight->shadoworigin, NULL, r_shadow_rtlight->radius + r_refdef.worldmodel->radius*2 + r_shadow_projectdistance.value, numshadowmark, shadowmarklist);
	}
	else if (numsurfaces)
...
void R_DrawRTLight(rtlight_t *rtlight, qboolean visible)
{
	...
	*/
	...
	else if (r_refdef.worldmodel && r_refdef.worldmodel->GetLightInfo)
	{
		...
		R_Shadow_EnlargeLeafSurfaceTrisBuffer(r_refdef.worldmodel->t.brush.num_leafs, r_refdef.worldmodel->num_surfaces, r_refdef.worldmodel->t.brush.shadowmesh ? r_refdef.worldmodel->t.brush.shadowmesh->numtriangles : r_refdef.worldmodel->surfmesh.num_triangles, r_refdef.worldmodel->surfmesh.num_triangles);
	...
	if (r_drawentities.integer)
	{
		for (i = 0;i < r_refdef.numentities;i++)
		{
			...
			if (r_viewcache.entityvisible[i] && model->DrawLight && (ent->flags & RENDER_LIGHT))
			{
				...
				if (r_refdef.worldmodel && r_refdef.worldmodel->t.brush.BoxTouchingLeafPVS && !r_refdef.worldmodel->t.brush.BoxTouchingLeafPVS(r_refdef.worldmodel, leafpvs, ent->mins, ent->maxs))
					continue;
...
void R_DrawModelShadows(void)
{
	...
	for (i = 0;i < r_refdef.numentities;i++)
	{
		...
		if (ent->model && ent->model->DrawShadowVolume != NULL && !ent->model->t.brush.submodel && (ent->flags & RENDER_SHADOW))
		{
...
void R_Shadow_UpdateWorldLight(dlight_t *light, vec3_t origin, vec3_t angles, vec3_t color, vec_t radius, vec_t corona, int style, int shadowenable, const char *cubemapname, vec_t coronasizescale, vec_t ambientscale, vec_t diffusescale, vec_t specularscale, int flags)
{
	...
	light->angles[0] = angles[0] - 360 * dr_floor(angles[0] / 360);
	light->angles[1] = angles[1] - 360 * dr_floor(angles[1] / 360);
	light->angles[2] = angles[2] - 360 * dr_floor(angles[2] / 360);
...
void R_Shadow_DrawLightSprite_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	intensity = 0.5;
	if (light->selected)
		intensity = 0.75 + 0.25 * dr_sin(realtime * M_PI * 4.0);
	if (!light->shadow)
		intensity *= 0.5f;
...
void R_Shadow_SelectLightInView(void)
{
	...
	for (light = r_shadow_worldlightchain;light;light = light->next)
	{
		...
		rating = (DotProduct(temp, r_view.forward) / dr_sqrt(DotProduct(temp, temp)));
		if (rating >= 0.95)
		{
			rating /= (1 + 0.0625f * dr_sqrt(DotProduct(temp, temp)));
	...
	if (lightsstring)
	{
		...
		while (*s)
		{
			...
			*s = tempchar;
			...
			radius = dr_sqrt(DotProduct(color, color) / (falloff * falloff * 8192.0f * 8192.0f));
...
void R_Shadow_LoadWorldLightsFromMap_LightArghliteTyrlite(void)
{
	...
	if (!data)
		data = r_refdef.worldmodel->t.brush.entities;
	if (!data)
		return;
...
void R_Shadow_SetCursorLocationForView(void)
{
	...
	r_editlights_cursorlocation[0] = dr_floor(endpos[0] / r_editlights_cursorgrid.value + 0.5f) * r_editlights_cursorgrid.value;
	r_editlights_cursorlocation[1] = dr_floor(endpos[1] / r_editlights_cursorgrid.value + 0.5f) * r_editlights_cursorgrid.value;
	r_editlights_cursorlocation[2] = dr_floor(endpos[2] / r_editlights_cursorgrid.value + 0.5f) * r_editlights_cursorgrid.value;
}
...
void R_Shadow_EditLights_Edit_f(void)
{
	...
	else if (!strcmp(Cmd_Argv(1), "colorscale"))
	{
		if (Cmd_Argc() == 3)
		{
			dr_real_t scale = atof(Cmd_Argv(2));
			color[0] *= scale;
			color[1] *= scale;
			color[2] *= scale;
		}
