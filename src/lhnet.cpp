...
int LHNET_Read(lhnetsocket_t *lhnetsocket, void *content, int maxcontentlength, lhnetaddress_t *address)
{
	...
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_INET4)
	{
		...
		value = recvfrom(lhnetsocket->inetsocket, (char *)content, maxcontentlength, 0, (struct sockaddr *)&address->addressdata.inet4, (int *)&inetaddresslength);
		if (value > 0)
		{
			address->addresstype = LHNETADDRESSTYPE_INET4;
			return value;
	...
	else if (lhnetsocket->address.addresstype == LHNETADDRESSTYPE_INET6)
	{
		...
		value = recvfrom(lhnetsocket->inetsocket, (char *)content, maxcontentlength, 0, (struct sockaddr *)&address->addressdata.inet6, (int *)&inetaddresslength);
		if (value > 0)
		{
			address->addresstype = LHNETADDRESSTYPE_INET6;
			return value;
...
int LHNET_Write(lhnetsocket_t *lhnetsocket, const void *content, int contentlength, const lhnetaddress_t *address)
{
	...
	{
		value = sendto(lhnetsocket->inetsocket, (const char*)content, contentlength, 0, (struct sockaddr *)&address->addressdata.inet4, sizeof(address->addressdata.inet4));
		if (value == -1)
		{
	...
	{
		value = sendto(lhnetsocket->inetsocket, (const char*)content, contentlength, 0, (struct sockaddr *)&address->addressdata.inet6, sizeof(address->addressdata.inet6));
		if (value == -1)
		{
			if (SOCKETERRNO == EWOULDBLOCK)
				return 0;
