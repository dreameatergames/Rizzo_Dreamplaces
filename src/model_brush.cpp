...
static mleaf_t *Mod_Q1BSP_PointInLeaf(model_t *model, const vec3_t p)
{
	...
	node = model->t.brush.data_nodes + model->brushq1.hulls[0].firstclipnode;
	while (node->plane)
...
static int Mod_Q1BSP_FindBoxClusters(model_t *model, const vec3_t mins, const vec3_t maxs, int maxclusters, int *clusterlist)
{
	...
	if (!model->t.brush.num_pvsclusters)
		return -1;
	node = model->t.brush.data_nodes;
	for (;;)
	{
#if 1
		if (node->plane)
...
static int Mod_Q1BSP_BoxTouchingLeafPVS(model_t *model, const unsigned char *pvs, const vec3_t mins, const vec3_t maxs)
{
	...
	for (;;)
	{
		...
		{
			// leaf - check cluster bit
			int clusterindex = ((mleaf_t *)node) - model->t.brush.data_leafs;
			if (CHECKPVSBIT(pvs, clusterindex))
			{
				// it is visible, return immediately with the news
				return true;
...
static int Mod_Q1BSP_BoxTouchingVisibleLeafs(model_t *model, const unsigned char *visibleleafs, const vec3_t mins, const vec3_t maxs)
{
	...
	for (;;)
	{
		...
		else
		{
			...
			if (visibleleafs[(mleaf_t *)node - model->t.brush.data_leafs])
			{
				// it is visible, return immediately with the news
				return true;
			}
						...
						else
						{
							...
							for (i = 0;i < 3;i++)
							{
								 ...
								 && f <= DotProduct(vert[1], edge[i]))
								{
									...
									dist = dr_sqrt(DotProduct(point, point));
									if (info->bestdist > dist)
									{
										info->bestdist = dist;
...
static void Mod_Q1BSP_FindNonSolidLocation(model_t *model, const vec3_t in, vec3_t out, float radius)
{
	...
	do
	{
		...
		Mod_Q1BSP_FindNonSolidLocation_r(&info, model->t.brush.data_nodes + model->brushq1.hulls[0].firstclipnode);
...
static int Mod_Q1BSP_RecursiveHullCheck(RecursiveHullCheckTraceInfo_t *t, int num, dr_real_t p1f, dr_real_t p2f, dr_real_t p1[3], dr_real_t p2[3])
{
...
static int Mod_Q1BSP_TraceLineOfSight_RecursiveNodeCheck(mnode_t *node, dr_real_t p1[3], dr_real_t p2[3])
{
	dr_real_t t1, t2;
	dr_real_t midf, mid[3];
	int ret, side;
...
{
	// this function currently only supports same size start and end
	dr_real_t tracestart[3], traceend[3];
	VectorCopy(start, tracestart);
	VectorCopy(end, traceend);
	return Mod_Q1BSP_TraceLineOfSight_RecursiveNodeCheck(model->t.brush.data_nodes, tracestart, traceend) != 2;
}
		...
		if (node->numsurfaces)
		{
			...
			for (i = 0;i < node->numsurfaces;i++, surface++)
			{
				...
				if (ds >= 0 && ds < surface->lightmapinfo->extents[0] && dt >= 0 && dt < surface->lightmapinfo->extents[1])
				{
					...
					size3 = lmwidth * lmheight
#ifndef SIMPLE_LIGHTS
					       	* 3
#endif
						; // LordHavoc: *3 for colored lighting

					lightmap = surface->lightmapinfo->samples + ((dt>>4) * lmwidth + (ds>>4))
#ifndef SIMPLE_LIGHTS
						*3
#endif
						; // LordHavoc: *3 for colored lighting
...
#else
						r00 += lightmap[0]*scale; g00+=r00; b00+=r00;
						r01 += lightmap[1]*scale; g01+=r01; b01+=r01;
						r10 += lightmap[line3+0]*scale; g10+=r10; b10+=r10;
						r11 += lightmap[line3+1]*scale; g11+=r11; b10+=r11;
#endif
						lightmap += size3;
					}
...
void Mod_Q1BSP_LightPoint(model_t *model, const vec3_t p, vec3_t ambientcolor, vec3_t diffusecolor, vec3_t diffusenormal)
{
	...
	Mod_Q1BSP_LightPoint_RecursiveBSPNode(model, ambientcolor, diffusecolor, diffusenormal, model->t.brush.data_nodes + model->brushq1.hulls[0].firstclipnode, p[0], p[1], p[2] + 0.125, p[2] - 65536);
}
...
void R_Q1BSP_LoadSplitSky (unsigned char *src, int width, int height, int bytesperpixel)
{
	...
	{
		loadmodel->t.brush.solidskytexture = R_LoadTexture2D(loadmodel->texturepool, "sky_solidtexture", width, height, src, bytesperpixel == 4 ? TEXTYPE_RGBA : TEXTYPE_PALETTE, TEXF_PRECACHE, bytesperpixel == 1 ? palette_complete : NULL);
		loadmodel->t.brush.alphaskytexture = NULL;
		return;
	}
	...
	loadmodel->t.brush.solidskytexture = R_LoadTexture2D(loadmodel->texturepool, "sky_solidtexture", 128, 128, (unsigned char *) solidpixels, TEXTYPE_RGBA, TEXF_PRECACHE, NULL);
	loadmodel->t.brush.alphaskytexture = R_LoadTexture2D(loadmodel->texturepool, "sky_alphatexture", 128, 128, (unsigned char *) alphapixels, TEXTYPE_RGBA, TEXF_ALPHA | TEXF_PRECACHE, NULL);
}
...
static void Mod_Q1BSP_LoadTextures(lump_t *l)
{
	...
	for (i = 0;i < m->nummiptex;i++)
	{
		...
		if (cls.state != ca_dedicated)
		{
			...
			if (!loadmodel->t.brush.ishlbsp && !strncmp(tx->name, "sky", 3) && mtwidth == 256 && mtheight == 128)
			{
			...
			{
				if (!Mod_LoadSkinFrame(&tx->skinframes[0],
#ifndef NO_GAMEMODE
						       	gamemode == GAME_TENEBRAE ? tx->name : 
#endif
							va("textures/%s/%s", mapname, tx->name), TEXF_MIPMAP | TEXF_ALPHA | TEXF_PRECACHE | TEXF_PICMIP, false, true)
				 && !Mod_LoadSkinFrame(&tx->skinframes[0],
#ifndef NO_GAMEMODE
							 gamemode == GAME_TENEBRAE ? tx->name : 
#endif
							 va("textures/%s", tx->name), TEXF_MIPMAP | TEXF_ALPHA | TEXF_PRECACHE | TEXF_PICMIP, false, true))
				{
					// did not find external texture, load it from the bsp or wad3
					if (loadmodel->t.brush.ishlbsp)
					{
...
static void Mod_Q1BSP_LoadLighting(lump_t *l)
{
	...
	if (loadmodel->t.brush.ishlbsp) // LordHavoc: load the colored lighting data straight
	{
#ifndef SIMPLE_LIGHTS
		...
		for(i=0;i<l->filelen;i+=3)
		{
			unsigned t;
			unsigned char r,g,b;
			r=mod_base[l->fileofs+i+0] >>= 1;
			g=mod_base[l->fileofs+i+1] >>= 1;
			b=mod_base[l->fileofs+i+2] >>= 1;
			t=r+g+b;
			t/=3;
			loadmodel->brushq1.lightdata[i/3]=t;
		}
#endif
	}
	else if (loadmodel->t.brush.ismcbsp)
	{
#ifndef SIMPLE_LIGHTS
		loadmodel->brushq1.lightdata = (unsigned char *)Mem_Alloc(loadmodel->mempool, l->filelen);
		...
		for(i=0;i<l->filelen;i+=3)
		{
			unsigned t;
			unsigned char r,g,b;
			r=mod_base[l->fileofs+i+0];
			g=mod_base[l->fileofs+i+1];
			b=mod_base[l->fileofs+i+2];
			t=r+g+b;
			t/=3;
			loadmodel->brushq1.lightdata[i/3]=t;
		}
...
#ifndef NO_Q3SHADERS
								loadmodel->t.brushq3.deluxemapping_modelspace = false;
								loadmodel->t.brushq3.deluxemapping = true;
#endif
							}
						}
						Mem_Free(data);
...
#else
					loadmodel->brushq1.lightdata = (unsigned char *)Mem_Alloc(loadmodel->mempool, (filesize - 8)/3);
					for(i=0;i<(filesize-8);i+=3)
					{
						unsigned t;
						unsigned char r,g,b;
						r=data[i+8];
...
{
	loadmodel->t.brush.entities = NULL;
	if (!l->filelen)
		return;
	loadmodel->t.brush.entities = (char *)Mem_Alloc(loadmodel->mempool, l->filelen);
	memcpy(loadmodel->t.brush.entities, mod_base + l->fileofs, l->filelen);
	if (loadmodel->t.brush.ishlbsp)
		Mod_Q1BSP_ParseWadsFromEntityLump(loadmodel->t.brush.entities);
}
...
static void Mod_Q1BSP_LoadSubmodels(lump_t *l, hullinfo_t *hullinfo)
{
	...
	loadmodel->t.brush.numsubmodels = count;
...
static void SubdividePolygon(int numverts, float *verts)
{
	...
	{
		m = (mins[i] + maxs[i]) * 0.5;
		m = r_subdivide_size.value * dr_floor(m/r_subdivide_size.value + 0.5);
		if (maxs[i] - m < 8)
			continue;
		if (m - mins[i] < 8)
			continue;
...
{
	unsigned copia_anterior=anterior;
	dface_t *in;
	msurface_t *surface;
...
#define LIGHTMAPSIZE 256
	...
	dbgf("Mod_Q1BSP_LoadFaces (0): %i, %iK\n",loadmodel->mempool->totalsize,(loadmodel->mempool->totalsize-anterior)/1024); anterior=loadmodel->mempool->totalsize;
	...
	dbgf("Mod_Q1BSP_LoadFaces (1): %i, %iK\n",loadmodel->mempool->totalsize,(loadmodel->mempool->totalsize-anterior)/1024); anterior=loadmodel->mempool->totalsize;
	...
	dbgf("Mod_Q1BSP_LoadFaces (2): %i, %iK\n",loadmodel->mempool->totalsize,(loadmodel->mempool->totalsize-anterior)/1024); anterior=loadmodel->mempool->totalsize;
	...
	dbgf("Mod_Q1BSP_LoadFaces count=%i, totalverts=%i, totaltris=%i\n",count,totalverts,totaltris);
	Mod_AllocSurfMesh(loadmodel->mempool, totalverts, totaltris, true, false, false);
	dbgf("Mod_Q1BSP_LoadFaces (3): %i, %iK\n",loadmodel->mempool->totalsize,(loadmodel->mempool->totalsize-anterior)/1024); anterior=loadmodel->mempool->totalsize;
	...
	for (surfacenum = 0, in = (dface_t *)(mod_base + l->fileofs), surface = loadmodel->data_surfaces;surfacenum < count;surfacenum++, in++, surface++)
	{
		...
		if ((unsigned int) planenum >= (unsigned int) loadmodel->t.brush.num_planes)
			Host_Error("Mod_Q1BSP_LoadFaces: invalid plane index %i (model has %i planes)", planenum, loadmodel->t.brush.num_planes);

		//surface->flags = surface->texture->flags;
		//if (LittleShort(in->side))
		//	surface->flags |= SURF_PLANEBACK;
		//surface->plane = loadmodel->t.brush.data_planes + planenum;
		...
		Mod_BuildTextureVectorsFromNormals(surface->num_firstvertex, surface->num_vertices, surface->num_triangles, loadmodel->surfmesh.data_vertex3f, loadmodel->surfmesh.data_texcoordtexture2f, loadmodel->surfmesh.data_normal3f, (loadmodel->surfmesh.data_element3i + 3 * surface->num_firsttriangle),
			       	...
			       	loadmodel->surfmesh.data_svector3f, loadmodel->surfmesh.data_tvector3f,
#else
				NULL, NULL,
#endif
			       	true);
		...
		for (i = 0;i < 2;i++)
		{
			surface->lightmapinfo->texturemins[i] = (int) dr_floor(texmins[i] / 16.0) * 16;
			surface->lightmapinfo->extents[i] = (int) dr_ceil(texmaxs[i] / 16.0) * 16 - surface->lightmapinfo->texturemins[i];
		}
		...
		if (i == -1)
		{
			...
			if ((surface->texture->basematerialflags & MATERIALFLAG_WATER) && (surface->lightmapinfo->texinfo->flags & TEX_SPECIAL) && ssize <= LIGHTMAPSIZE && tsize <= LIGHTMAPSIZE)
			{
				surface->lightmapinfo->samples = (unsigned char *)Mem_Alloc(loadmodel->mempool, ssize * tsize
#ifndef SIMPLE_LIGHTS
					       	* 3
#endif
						);
				surface->lightmapinfo->styles[0] = 0;
				memset(surface->lightmapinfo->samples, 128, ssize * tsize
#ifndef SIMPLE_LIGHTS
					       	* 3
#endif
						);
		...
		else if (loadmodel->t.brush.ishlbsp) // LordHavoc: HalfLife map (bsp version 30)
		...
		if (!(surface->lightmapinfo->texinfo->flags & TEX_SPECIAL) || surface->lightmapinfo->samples)
		{
			...
			if (ssize > LIGHTMAPSIZE || tsize > LIGHTMAPSIZE)
			...
			if (!lightmaptexture || !Mod_Q1BSP_AllocLightmapBlock(lightmap_lineused, LIGHTMAPSIZE, LIGHTMAPSIZE, ssize, tsize, &lightmapx, &lightmapy))
			{
				...
				lightmaptexture = R_LoadTexture2D(loadmodel->texturepool, va("lightmap%i", lightmapnumber), LIGHTMAPSIZE, LIGHTMAPSIZE, NULL,
#ifndef SIMPLE_LIGHTS
		   loadmodel->brushq1.lightmaprgba ? TEXTYPE_RGBA : TEXTYPE_RGB,
#else
		   TEXTYPE_RGB,
#endif
		   TEXF_FORCELINEAR | TEXF_PRECACHE, NULL);
	...
	dbgf("Mod_Q1BSP_LoadFaces (4): %i, %iK\n",loadmodel->mempool->totalsize,(loadmodel->mempool->totalsize-anterior)/1024); anterior=loadmodel->mempool->totalsize;
anterior=copia_anterior;
}
...
static void Mod_Q1BSP_LoadNodes_RecursiveSetParent(mnode_t *node, mnode_t *parent)
{
	...
	else
	{
		...
		for (j = 0;j < leaf->numleafbrushes;j++)
			leaf->combinedsupercontents |= loadmodel->t.brush.data_brushes[leaf->firstleafbrush[j]].texture->supercontents;
...
static void Mod_Q1BSP_LoadNodes(lump_t *l)
{
	...
	loadmodel->t.brush.data_nodes = out;
	loadmodel->t.brush.num_nodes = count;
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->plane = loadmodel->t.brush.data_planes + p;
		...
		for (j=0 ; j<2 ; j++)
		{
			...
			if (p >= 0)
				out->children[j] = loadmodel->t.brush.data_nodes + p;
			else
				out->children[j] = (mnode_t *)(loadmodel->t.brush.data_leafs + (-1 - p));
		}
	}

	Mod_Q1BSP_LoadNodes_RecursiveSetParent(loadmodel->t.brush.data_nodes, NULL);	// sets nodes and leafs
}
...
static void Mod_Q1BSP_LoadLeafs(lump_t *l)
{
	...
	loadmodel->t.brush.data_leafs = out;
	loadmodel->t.brush.num_leafs = count;
	// get visleafs from the submodel data
	loadmodel->t.brush.num_pvsclusters = loadmodel->brushq1.submodels[0].visleafs;
	loadmodel->t.brush.num_pvsclusterbytes = (loadmodel->t.brush.num_pvsclusters+7)>>3;
	loadmodel->t.brush.data_pvsclusters = (unsigned char *)Mem_Alloc(loadmodel->mempool, loadmodel->t.brush.num_pvsclusters * loadmodel->t.brush.num_pvsclusterbytes);
	memset(loadmodel->t.brush.data_pvsclusters, 0xFF, loadmodel->t.brush.num_pvsclusters * loadmodel->t.brush.num_pvsclusterbytes);
	...
	for ( i=0 ; i<count ; i++, in++, out++)
	{
		...
		out->firstleafsurface = loadmodel->t.brush.data_leafsurfaces + LittleShort(in->firstmarksurface);
		out->numleafsurfaces = LittleShort(in->nummarksurfaces);
		if (out->firstleafsurface < 0 || LittleShort(in->firstmarksurface) + out->numleafsurfaces > loadmodel->t.brush.num_leafsurfaces)
		{
			Con_Printf("Mod_Q1BSP_LoadLeafs: invalid leafsurface range %i:%i outside range %i:%i\n", (int)(out->firstleafsurface - loadmodel->t.brush.data_leafsurfaces), (int)(out->firstleafsurface + out->numleafsurfaces - loadmodel->t.brush.data_leafsurfaces), 0, loadmodel->t.brush.num_leafsurfaces);
			out->firstleafsurface = NULL;
			out->numleafsurfaces = 0;
		}

		out->clusterindex = i - 1;
		if (out->clusterindex >= loadmodel->t.brush.num_pvsclusters)
			out->clusterindex = -1;
		...
		if (p >= 0 && out->clusterindex >= 0)
		{
			if (p >= loadmodel->brushq1.num_compressedpvs)
				...
				Mod_Q1BSP_DecompressVis(loadmodel->brushq1.data_compressedpvs + p, loadmodel->brushq1.data_compressedpvs + loadmodel->brushq1.num_compressedpvs, loadmodel->t.brush.data_pvsclusters + out->clusterindex * loadmodel->t.brush.num_pvsclusterbytes, loadmodel->t.brush.data_pvsclusters + (out->clusterindex + 1) * loadmodel->t.brush.num_pvsclusterbytes);
		}
...
qboolean Mod_Q1BSP_CheckWaterAlphaSupport(void)
{
	...
	for (i = 0, leaf = loadmodel->t.brush.data_leafs;i < loadmodel->t.brush.num_leafs;i++, leaf++)
	{
		if ((leaf->contents == CONTENTS_WATER || leaf->contents == CONTENTS_SLIME) && (leaf->clusterindex >= 0 && loadmodel->t.brush.data_pvsclusters))
		{
			pvs = loadmodel->t.brush.data_pvsclusters + leaf->clusterindex * loadmodel->t.brush.num_pvsclusterbytes;
			for (j = 0;j < loadmodel->t.brush.num_leafs;j++)
				if (CHECKPVSBIT(pvs, loadmodel->t.brush.data_leafs[j].clusterindex) && loadmodel->t.brush.data_leafs[j].contents == CONTENTS_EMPTY)
					return true;
		}
	}
	return false;
...
static void Mod_Q1BSP_LoadClipnodes(lump_t *l, hullinfo_t *hullinfo)
{
	...
	for (i = 1; i < hullinfo->numhulls; i++)
	{
		...
		hull->planes = loadmodel->t.brush.data_planes;
	...
	for (i=0 ; i<count ; i++, out++, in++)
	{
		...
		if (out->planenum < 0 || out->planenum >= loadmodel->t.brush.num_planes)
...
static void Mod_Q1BSP_MakeHull0(void)
{
	...
	in = loadmodel->t.brush.data_nodes;
	out = (dclipnode_t *)Mem_Alloc(loadmodel->mempool, loadmodel->t.brush.num_nodes * sizeof(dclipnode_t));

	hull->clipnodes = out;
	hull->firstclipnode = 0;
	hull->lastclipnode = loadmodel->t.brush.num_nodes - 1;
	hull->planes = loadmodel->t.brush.data_planes;

	for (i = 0;i < loadmodel->t.brush.num_nodes;i++, out++, in++)
	{
		out->planenum = in->plane - loadmodel->t.brush.data_planes;
		out->children[0] = in->children[0]->plane ? in->children[0] - loadmodel->t.brush.data_nodes : ((mleaf_t *)in->children[0])->contents;
		out->children[1] = in->children[1]->plane ? in->children[1] - loadmodel->t.brush.data_nodes : ((mleaf_t *)in->children[1])->contents;
	}
}
...
static void Mod_Q1BSP_LoadLeaffaces(lump_t *l)
{
	...
	loadmodel->t.brush.num_leafsurfaces = l->filelen / sizeof(*in);
	loadmodel->t.brush.data_leafsurfaces = (int *)Mem_Alloc(loadmodel->mempool, loadmodel->t.brush.num_leafsurfaces * sizeof(int));

	for (i = 0;i < loadmodel->t.brush.num_leafsurfaces;i++)
	{
		j = (unsigned) LittleShort(in[i]);
		if (j >= loadmodel->num_surfaces)
			Host_Error("Mod_Q1BSP_LoadLeaffaces: bad surface number");
		loadmodel->t.brush.data_leafsurfaces[i] = j;
	}
}
...
static void Mod_Q1BSP_LoadPlanes(lump_t *l)
{
	...
	loadmodel->t.brush.num_planes = l->filelen / sizeof(*in);
	loadmodel->t.brush.data_planes = out = (mplane_t *)Mem_Alloc(loadmodel->mempool, loadmodel->t.brush.num_planes * sizeof(*out));

	for (i = 0;i < loadmodel->t.brush.num_planes;i++, in++, out++)
	{
		...
		for (;;)
		{
			...
			if (com_token[0] == '{')
			{
				...
				if (firstbrush)
				{
					if (submodel)
					{
						if (submodel > loadmodel->t.brush.numsubmodels)
						{
							Con_Printf("Mod_Q1BSP_LoadMapBrushes: .map has more submodels than .bsp!\n");
							model = NULL;
						}
						else
							model = loadmodel->t.brush.submodels[submodel];
					}
					else
						model = loadmodel;
				}
...
typedef struct portal_s
{
	...
	int numpoints;
	dr_real_t points[3*MAX_PORTALPOINTS];
...
static portal_t *AllocPortal(void)
{
	...
	memset((void *)p,0,sizeof(portal_t));
	p->chain = portalchain;
	portalchain = p;
	return p;
}
...
static void Mod_Q1BSP_FinalizePortals(void)
{
	...
	leaf = loadmodel->t.brush.data_leafs;
	endleaf = leaf + loadmodel->t.brush.num_leafs;
	...
	loadmodel->t.brush.data_portals = (mportal_t *)Mem_Alloc(loadmodel->mempool, numportals * sizeof(mportal_t) + numpoints * sizeof(mvertex_t));
	loadmodel->t.brush.num_portals = numportals;
	loadmodel->t.brush.data_portalpoints = (mvertex_t *)((unsigned char *) loadmodel->t.brush.data_portals + numportals * sizeof(mportal_t));
	loadmodel->t.brush.num_portalpoints = numpoints;
	// clear all leaf portal chains
	for (i = 0;i < loadmodel->t.brush.num_leafs;i++)
		loadmodel->t.brush.data_leafs[i].portals = NULL;
	// process all portals in the global portal chain, while freeing them
	portal = loadmodel->t.brush.data_portals;
	point = loadmodel->t.brush.data_portalpoints;
	p = portalchain;
	portalchain = NULL;
	while (p)
	{
	...
	Mod_Q1BSP_RecursiveRecalcNodeBBox(loadmodel->t.brush.data_nodes);
}
#endif
...
static dr_real_t _mq1_frontpoints[3*MAX_PORTALPOINTS];
static dr_real_t _mq1_backpoints[3*MAX_PORTALPOINTS];

#ifndef ONE_PORTAL_PASS
static void Mod_Q1BSP_NodePortals(mnode_t *real_node)
{
	int i, side;
	mnode_t *front, *back, *other_node, *ret, *node, *used;
	mplane_t clipplane, *plane;
	portal_t *portal, *nextportal, *nodeportal, *splitportal, *temp;
	int numfrontpoints, numbackpoints;

	ret=real_node->parent;
	real_node->parent=NULL;
	node=real_node;
...
//printf("NODE: %p (%p %p)\n",node,front,back);
		...
		if (front && front == back)
			Host_Error("Mod_Q1BSP_NodePortals: corrupt node hierarchy");
	       	if (plane=node->plane)
		{
nodeportal = AllocPortal();
//			memset((void *)nodeportal,0,sizeof(portal_t));
			...
			for (portal = (portal_t *)node->portals;portal;portal = portal->next[side])
			{
				...
				if (portal->nodes[0] == portal->nodes[1])
					Host_Error("Mod_Q1BSP_NodePortals: portal has same node on both sides(1)");
				if (portal->nodes[0] == node)
					side = 0;
				else if (portal->nodes[1] == node)
				{
					...
					Host_Error("Mod_Q1BSP_NodePortals: mislinked portal");
		
					for (i = 0;i < nodeportal->numpoints*3;i++)
				{
					_mq1_frontpoints[i] = nodeportal->points[i];
				}
				PolygonD_Divide(nodeportal->numpoints, _mq1_frontpoints, clipplane.normal[0], clipplane.normal[1], clipplane.normal[2], clipplane.dist, PORTAL_DIST_EPSILON, MAX_PORTALPOINTS, nodeportal->points, &nodeportal->numpoints, 0, NULL, NULL, NULL);
			...
			{
				Con_Print("Mod_Q1BSP_NodePortals: WARNING: new portal was clipped away\n");
				nodeportal->numpoints = 0;
			}
			else if (nodeportal->numpoints >= MAX_PORTALPOINTS)
			{
				Con_Print("Mod_Q1BSP_NodePortals: WARNING: new portal has too many points\n");
				nodeportal->numpoints = 0;
			}
			...
			for (portal = (portal_t *)node->portals;portal;portal = nextportal)
			{
				...
				PolygonD_Divide(portal->numpoints, portal->points, plane->normal[0], plane->normal[1], plane->normal[2], plane->dist, PORTAL_DIST_EPSILON, MAX_PORTALPOINTS, _mq1_frontpoints, &numfrontpoints, MAX_PORTALPOINTS, _mq1_backpoints, &numbackpoints, NULL);
				...
				// the portal is split
splitportal = AllocPortal();
//				memset((void *)splitportal,0,sizeof(portal_t));
				temp = splitportal->chain;
				*splitportal = *portal;
				splitportal->chain = temp;
				for (i = 0;i < numbackpoints*3;i++)
					splitportal->points[i] = _mq1_backpoints[i];
				splitportal->numpoints = numbackpoints;
				for (i = 0;i < numfrontpoints*3;i++)
					portal->points[i] = _mq1_frontpoints[i];
		...
		if (plane && front && back!=used && front->plane)
		{
			front->parent=node;
			node=front;
		}
		else if (plane && back && back!=used && back->plane)
		{
			back->parent=node;
			node=back;
		}
		else
		{
			...
			{
				if (used==node->children[0] && node->children[1] && node->children[1] && node->children[1]->plane)
				{
					used=node;
					node=node->children[1];
					node->parent=used;
...
#define ONE_PASS_MAX_PORTALS 8192
#define ONE_PASS_MAX_POINTS 32768 

static void Mod_Q1BSP_OnePortalPass(mnode_t *real_node)
{
	int i, j, side;
	mnode_t *front, *back, *other_node, *ret, *node, *used;
	mplane_t clipplane, *plane;
	portal_t *portal, *nextportal, *nodeportal, *splitportal, *temp;
	portal_t portalbuf[MAX_PORTALPOINTS*2];
	int portalbuf_index=0, numfrontpoints, numbackpoints;
	mleaf_t *leaf, *endleaf;
	mportal_t *finalize_portal;
	mvertex_t *finalize_point;
	...
	loadmodel->t.brush.data_portals = (mportal_t *)Mem_Alloc(loadmodel->mempool, ONE_PASS_MAX_PORTALS * sizeof(mportal_t) + ONE_PASS_MAX_POINTS * sizeof(mvertex_t));
	loadmodel->t.brush.num_portals = ONE_PASS_MAX_PORTALS;
	loadmodel->t.brush.data_portalpoints = (mvertex_t *)((unsigned char *) loadmodel->t.brush.data_portals + ONE_PASS_MAX_PORTALS * sizeof(mportal_t));
	loadmodel->t.brush.num_portalpoints = ONE_PASS_MAX_POINTS;
	// clear all leaf portal chains
	for (i = 0;i < loadmodel->t.brush.num_leafs;i++)
		loadmodel->t.brush.data_leafs[i].portals = NULL;
	// process all portals in the global portal chain, while freeing them
	finalize_portal = loadmodel->t.brush.data_portals;
	finalize_point = loadmodel->t.brush.data_portalpoints;
...
printf("NODE: %p (%p %p)\n",node,front,back);
		...
		if (front && front == back)
			Host_Error("Mod_Q1BSP_OnePortalPass: corrupt node hierarchy");
	       	if (plane=node->plane)
		{
			nodeportal=&portalbuf[portalbuf_index++];
			if (portalbuf_index>(MAX_PORTALPOINTS*2))
				portalbuf_index=0;
			memset((void *)nodeportal,0,sizeof(portal_t));
			nodeportal->plane = *plane;
			PolygonD_QuadForPlane(nodeportal->points, nodeportal->plane.normal[0], nodeportal->plane.normal[1], nodeportal->plane.normal[2], nodeportal->plane.dist, 1024.0*1024.0*1024.0);
			...
			for (portal = (portal_t *)node->portals;portal;portal = portal->next[side])
			{
				clipplane = portal->plane;
				if (portal->nodes[0] == portal->nodes[1])
					Host_Error("Mod_Q1BSP_OnePortalPass: portal has same node on both sides(1)");
				if (portal->nodes[0] == node)
					side = 0;
				else if (portal->nodes[1] == node)
				{
					...
					Host_Error("Mod_Q1BSP_OnePortalPass: mislinked portal");
			...
			if (nodeportal->numpoints < 3)
			{
				Con_Print("Mod_Q1BSP_OnePortalPass: WARNING: new portal was clipped away\n");
				nodeportal->numpoints = 0;
			}
			else if (nodeportal->numpoints >= MAX_PORTALPOINTS)
			{
				Con_Print("Mod_Q1BSP_OnePortalPass: WARNING: new portal has too many points\n");
				nodeportal->numpoints = 0;
			}
			...
			{
				if (portal->nodes[0] == portal->nodes[1])
					Host_Error("Mod_Q1BSP_OnePortalPass: portal has same node on both sides(2)");
				if (portal->nodes[0] == node)
					side = 0;
				else if (portal->nodes[1] == node)
					side = 1;
				...
				// the portal is split
				splitportal=&portalbuf[portalbuf_index++];
				if (portalbuf_index>(MAX_PORTALPOINTS*2))
					portalbuf_index=0;
				memset((void *)splitportal,0,sizeof(portal_t));
				temp = splitportal->chain;
		...
		if (nodeportal->numpoints >= 3 && nodeportal->nodes[0] != nodeportal->nodes[1])
		{
			// note: this check must match the one above or it will usually corrupt memory
			// the nodes[0] != nodes[1] check is because leaf 0 is the shared solid leaf, it can have many portals inside with leaf 0 on both sides
			if (((mleaf_t *)nodeportal->nodes[0])->clusterindex >= 0 && ((mleaf_t *)nodeportal->nodes[1])->clusterindex >= 0)
			{
				// first make the back to front portal(forward portal)
				finalize_portal->points = finalize_point;
				finalize_portal->numpoints = nodeportal->numpoints;
				finalize_portal->plane.dist = nodeportal->plane.dist;
				VectorCopy(nodeportal->plane.normal, finalize_portal->plane.normal);
				finalize_portal->here = (mleaf_t *)nodeportal->nodes[1];
				finalize_portal->past = (mleaf_t *)nodeportal->nodes[0];
				// copy points
				for (j = 0;j < finalize_portal->numpoints;j++)
				{
					VectorCopy(nodeportal->points + j*3, finalize_point->position);
					finalize_point++;
				}
				BoxFromPoints(finalize_portal->mins, finalize_portal->maxs, finalize_portal->numpoints, finalize_portal->points->position);
				PlaneClassify(&finalize_portal->plane);

				// link into leaf's portal chain
				finalize_portal->next = finalize_portal->here->portals;
				finalize_portal->here->portals = finalize_portal;
			...
			for (i = 0;i < 2;i++)
			{
				leaf = (mleaf_t *)nodeportal->nodes[i];
				for (j = 0;j < nodeportal->numpoints;j++)
				{
					if (leaf->mins[0] > nodeportal->points[j*3+0]) leaf->mins[0] = nodeportal->points[j*3+0];
					if (leaf->mins[1] > nodeportal->points[j*3+1]) leaf->mins[1] = nodeportal->points[j*3+1];
					if (leaf->mins[2] > nodeportal->points[j*3+2]) leaf->mins[2] = nodeportal->points[j*3+2];
					if (leaf->maxs[0] < nodeportal->points[j*3+0]) leaf->maxs[0] = nodeportal->points[j*3+0];
					if (leaf->maxs[1] < nodeportal->points[j*3+1]) leaf->maxs[1] = nodeportal->points[j*3+1];
					if (leaf->maxs[2] < nodeportal->points[j*3+2]) leaf->maxs[2] = nodeportal->points[j*3+2];
				}
			}
		}
	}
...
{
	portalchain = NULL;
	memset((void *)&_mq1_frontpoints[0],0,sizeof(_mq1_frontpoints));
	memset((void *)&_mq1_backpoints[0],0,sizeof(_mq1_backpoints));

#ifndef ONE_PORTAL_PASS
	dbg("Mod_Q1BSP_MakePortals 0");
#if 0
	Mod_Q1BSP_RecursiveNodePortals(loadmodel->t.brush.data_nodes);
#endif
	Mod_Q1BSP_NodePortals(loadmodel->t.brush.data_nodes);
	dbg("Mod_Q1BSP_MakePortals 1");
	Mod_Q1BSP_FinalizePortals();
	dbg("Mod_Q1BSP_MakePortals 2");
#else
	Mod_Q1BSP_OnePortalPass(loadmodel->t.brush.data_nodes);
#endif
}
...
static unsigned char *Mod_Q1BSP_GetPVS(model_t *model, const vec3_t p)
{
	...
	if (((mleaf_t *)node)->clusterindex >= 0)
		return model->t.brush.data_pvsclusters + ((mleaf_t *)node)->clusterindex * model->t.brush.num_pvsclusterbytes;
	else
		return NULL;
}
...
static void Mod_Q1BSP_FatPVS_RecursiveBSPNode(model_t *model, const vec3_t org, vec_t radius, unsigned char *pvsbuffer, int pvsbytes, mnode_t *node)
{
	...
	{
		int i;
		unsigned char *pvs = model->t.brush.data_pvsclusters + ((mleaf_t *)node)->clusterindex * model->t.brush.num_pvsclusterbytes;
...
{
	int bytes = model->t.brush.num_pvsclusterbytes;
	bytes = min(bytes, pvsbufferlength);
	if (r_novis.integer || !model->t.brush.num_pvsclusters || !Mod_Q1BSP_GetPVS(model, org))
	{
		memset(pvsbuffer, 0xFF, bytes);
		return bytes;
	}
	memset(pvsbuffer, 0, bytes);
	Mod_Q1BSP_FatPVS_RecursiveBSPNode(model, org, radius, pvsbuffer, bytes, model->t.brush.data_nodes);
	return bytes;
}
...
static void Mod_Q1BSP_RoundUpToHullSize(model_t *cmodel, const vec3_t inmins, const vec3_t inmaxs, vec3_t outmins, vec3_t outmaxs)
{
	...
	else if (cmodel->t.brush.ishlbsp)
	{
		if (size[0] < 3)
			hull = &cmodel->brushq1.hulls[0]; // 0x0x0
		else if (size[0] <= 32)
...
void Mod_Q1BSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (!memcmp (buffer, "MCBSPpad", 8))
	{
		...
		mod->t.brush.ismcbsp = true;
		mod->t.brush.ishlbsp = false;
	...
	else
	{
		...
		mod->t.brush.ishlbsp = i == 30;
		mod->t.brush.ismcbsp = false;

	// fill in hull info
		VectorClear (hullinfo.hullsizes[0][0]);
		VectorClear (hullinfo.hullsizes[0][1]);
		if (mod->t.brush.ishlbsp)
		{
			hullinfo.numhulls = 4;
	...
	dbgf("Mod_Q1BSP_Load %s (0): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	mod->soundfromcenter = true;
	mod->TraceBox = Mod_Q1BSP_TraceBox;
	mod->t.brush.TraceLineOfSight = Mod_Q1BSP_TraceLineOfSight;
	mod->t.brush.SuperContentsFromNativeContents = Mod_Q1BSP_SuperContentsFromNativeContents;
	mod->t.brush.NativeContentsFromSuperContents = Mod_Q1BSP_NativeContentsFromSuperContents;
	mod->t.brush.GetPVS = Mod_Q1BSP_GetPVS;
	mod->t.brush.FatPVS = Mod_Q1BSP_FatPVS;
	mod->t.brush.BoxTouchingPVS = Mod_Q1BSP_BoxTouchingPVS;
	mod->t.brush.BoxTouchingLeafPVS = Mod_Q1BSP_BoxTouchingLeafPVS;
	mod->t.brush.BoxTouchingVisibleLeafs = Mod_Q1BSP_BoxTouchingVisibleLeafs;
	mod->t.brush.FindBoxClusters = Mod_Q1BSP_FindBoxClusters;
	mod->t.brush.LightPoint = Mod_Q1BSP_LightPoint;
	mod->t.brush.FindNonSolidLocation = Mod_Q1BSP_FindNonSolidLocation;
	mod->t.brush.AmbientSoundLevelsForPoint = Mod_Q1BSP_AmbientSoundLevelsForPoint;
	mod->t.brush.RoundUpToHullSize = Mod_Q1BSP_RoundUpToHullSize;
	mod->t.brush.PointInLeaf = Mod_Q1BSP_PointInLeaf;

	if (loadmodel->isworldmodel)
	{
		Cvar_SetValue("halflifebsp", mod->t.brush.ishlbsp);
		Cvar_SetValue("mcbsp", mod->t.brush.ismcbsp);
	}
...
// load into heap
	...
	mod->t.brush.qw_md4sum = 0;
	mod->t.brush.qw_md4sum2 = 0;
	for (i = 0;i < HEADER_LUMPS;i++)
	{
		if (i == LUMP_ENTITIES)
			continue;
		mod->t.brush.qw_md4sum ^= LittleLong(Com_BlockChecksum(mod_base + header->lumps[i].fileofs, header->lumps[i].filelen));
		if (i == LUMP_VISIBILITY || i == LUMP_LEAFS || i == LUMP_NODES)
			continue;
		mod->t.brush.qw_md4sum2 ^= LittleLong(Com_BlockChecksum(mod_base + header->lumps[i].fileofs, header->lumps[i].filelen));
	}

	dbgf("Mod_Q1BSP_Load %s (1): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadEntities(&header->lumps[LUMP_ENTITIES]);
	dbgf("Mod_Q1BSP_Load %s (2): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadVertexes(&header->lumps[LUMP_VERTEXES]);
	dbgf("Mod_Q1BSP_Load %s (3): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadEdges(&header->lumps[LUMP_EDGES]);
	dbgf("Mod_Q1BSP_Load %s (4): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadSurfedges(&header->lumps[LUMP_SURFEDGES]);
	dbgf("Mod_Q1BSP_Load %s (5): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadTextures(&header->lumps[LUMP_TEXTURES]);
	dbgf("Mod_Q1BSP_Load %s (6): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadLighting(&header->lumps[LUMP_LIGHTING]);
	dbgf("Mod_Q1BSP_Load %s (7): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadPlanes(&header->lumps[LUMP_PLANES]);
	dbgf("Mod_Q1BSP_Load %s (8): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadTexinfo(&header->lumps[LUMP_TEXINFO]);
	dbgf("Mod_Q1BSP_Load %s (9): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadFaces(&header->lumps[LUMP_FACES]);
	dbgf("Mod_Q1BSP_Load %s (10): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadLeaffaces(&header->lumps[LUMP_MARKSURFACES]);
	dbgf("Mod_Q1BSP_Load %s (11): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadVisibility(&header->lumps[LUMP_VISIBILITY]);
	dbgf("Mod_Q1BSP_Load %s (12): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	...
	dbgf("Mod_Q1BSP_Load %s (13): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadLeafs(&header->lumps[LUMP_LEAFS]);
	dbgf("Mod_Q1BSP_Load %s (14): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadNodes(&header->lumps[LUMP_NODES]);
	dbgf("Mod_Q1BSP_Load %s (15): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_LoadClipnodes(&header->lumps[LUMP_CLIPNODES], &hullinfo);
	dbgf("Mod_Q1BSP_Load %s (16): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;

	// check if the map supports transparent water rendering
	loadmodel->t.brush.supportwateralpha = Mod_Q1BSP_CheckWaterAlphaSupport();
	dbgf("Mod_Q1BSP_Load %s (17): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;

	if (mod->brushq1.data_compressedpvs)
		Mem_Free(mod->brushq1.data_compressedpvs);
	dbgf("Mod_Q1BSP_Load %s (18): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	mod->brushq1.data_compressedpvs = NULL;
	mod->brushq1.num_compressedpvs = 0;

	Mod_Q1BSP_MakeHull0();
	dbgf("Mod_Q1BSP_Load %s (19): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_Q1BSP_MakePortals();
	dbgf("Mod_Q1BSP_Load %s (20): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	...
	loadmodel->t.brush.shadowmesh = Mod_ShadowMesh_Begin(loadmodel->mempool, numshadowmeshtriangles * 3, numshadowmeshtriangles, NULL, NULL, NULL, false, false, true);
	for (j = 0, surface = loadmodel->data_surfaces;j < loadmodel->num_surfaces;j++, surface++)
		Mod_ShadowMesh_AddMesh(loadmodel->mempool, loadmodel->t.brush.shadowmesh, NULL, NULL, NULL, loadmodel->surfmesh.data_vertex3f, NULL, NULL, NULL, NULL, surface->num_triangles, (loadmodel->surfmesh.data_element3i + 3 * surface->num_firsttriangle));
	loadmodel->t.brush.shadowmesh = Mod_ShadowMesh_Finish(loadmodel->mempool, loadmodel->t.brush.shadowmesh, false, true, false);
	dbgf("Mod_Q1BSP_Load %s (21): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;
	Mod_BuildTriangleNeighbors(loadmodel->t.brush.shadowmesh->neighbor3i, loadmodel->t.brush.shadowmesh->element3i, loadmodel->t.brush.shadowmesh->numtriangles);
	dbgf("Mod_Q1BSP_Load %s (22): %i, %iK\n",mod->name,mod->mempool->totalsize,(mod->mempool->totalsize-anterior)/1024); anterior=mod->mempool->totalsize;

	if (loadmodel->t.brush.numsubmodels)
		loadmodel->t.brush.submodels = (model_t **)Mem_Alloc(loadmodel->mempool, loadmodel->t.brush.numsubmodels * sizeof(model_t *));
	...
	for (i = 0;i < mod->t.brush.numsubmodels;i++)
	{
		...
		mod->t.brush.submodel = i;

		if (loadmodel->t.brush.submodels)
			loadmodel->t.brush.submodels[i] = mod;
		...
		if (i != 0)
		{
			mod->t.brush.TraceLineOfSight = NULL;
			mod->t.brush.GetPVS = NULL;
			mod->t.brush.FatPVS = NULL;
			mod->t.brush.BoxTouchingPVS = NULL;
			mod->t.brush.BoxTouchingLeafPVS = NULL;
			mod->t.brush.BoxTouchingVisibleLeafs = NULL;
			mod->t.brush.FindBoxClusters = NULL;
			mod->t.brush.LightPoint = NULL;
			mod->t.brush.AmbientSoundLevelsForPoint = NULL;
		}
		...
		if (mod->nummodelsurfaces)
		{
			...
			modelyawradius = dr_sqrt(modelyawradius);
			modelradius = dr_sqrt(modelradius);
	...
	if (developer.integer >= 10)
		Con_Printf("Some stats for q1bsp model \"%s\": %i faces, %i nodes, %i leafs, %i visleafs, %i visleafportals\n", loadmodel->name, loadmodel->num_surfaces, loadmodel->t.brush.num_nodes, loadmodel->t.brush.num_leafs, mod->t.brush.num_pvsclusters, loadmodel->t.brush.num_portals);
#endif
}
...
void static Mod_Q2BSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	for (i = 0;i < Q2HEADER_LUMPS;i++)
	{
		if (i == Q2LUMP_ENTITIES)
			continue;
		mod->t.brush.qw_md4sum ^= Com_BlockChecksum(mod_base + header->lumps[i].fileofs, header->lumps[i].filelen);
		if (i == Q2LUMP_VISIBILITY || i == Q2LUMP_LEAFS || i == Q2LUMP_NODES)
			continue;
		mod->t.brush.qw_md4sum2 ^= Com_BlockChecksum(mod_base + header->lumps[i].fileofs, header->lumps[i].filelen);
	}
...
static void Mod_Q3BSP_LoadEntities(lump_t *l)
{
	...
	float v[3];
	loadmodel->t.brushq3.num_lightgrid_cellsize[0] = 64;
	loadmodel->t.brushq3.num_lightgrid_cellsize[1] = 64;
	loadmodel->t.brushq3.num_lightgrid_cellsize[2] = 128;
	if (!l->filelen)
		return;
	loadmodel->t.brush.entities = (char *)Mem_Alloc(loadmodel->mempool, l->filelen);
	memcpy(loadmodel->t.brush.entities, mod_base + l->fileofs, l->filelen);
	data = loadmodel->t.brush.entities;
		...
		while (1)
		{
			...
			if (!strcmp("gridsize", key))
			{
				if (sscanf(value, "%f %f %f", &v[0], &v[1], &v[2]) == 3 && v[0] != 0 && v[1] != 0 && v[2] != 0)
					VectorCopy(v, loadmodel->t.brushq3.num_lightgrid_cellsize);
			}
		}
	}
}
		...
		if (shader)
		{
			...
			if (shader->surfaceparms & Q3SURFACEPARM_SKY)
			{
				...
				if (shader->skyboxname[0])
				{
					...
					dpsnprintf(loadmodel->t.brush.skybox, sizeof(loadmodel->t.brush.skybox), "%s_", shader->skyboxname);
				}
			}
...
static void Mod_Q3BSP_LoadPlanes(lump_t *l)
{
	...
	loadmodel->t.brush.data_planes = out;
	loadmodel->t.brush.num_planes = count;
...
static void Mod_Q3BSP_LoadBrushSides(lump_t *l)
{
	...
	loadmodel->t.brush.data_brushsides = out;
	loadmodel->t.brush.num_brushsides = count;

	for (i = 0;i < count;i++, in++, out++)
	{
		n = LittleLong(in->planeindex);
		if (n < 0 || n >= loadmodel->t.brush.num_planes)
			Host_Error("Mod_Q3BSP_LoadBrushSides: invalid planeindex %i (%i planes)", n, loadmodel->t.brush.num_planes);
		out->plane = loadmodel->t.brush.data_planes + n;
...
static void Mod_Q3BSP_LoadBrushes(lump_t *l)
{
	...
	loadmodel->t.brush.data_brushes = out;
	loadmodel->t.brush.num_brushes = count;
	...
	for (i = 0;i < count;i++, in++, out++)
	{
		...
		if (n < 0 || n + c > loadmodel->t.brush.num_brushsides)
			Host_Error("Mod_Q3BSP_LoadBrushes: invalid brushside range %i : %i (%i brushsides)", n, n + c, loadmodel->t.brush.num_brushsides);
		out->firstbrushside = loadmodel->t.brush.data_brushsides + n;
		out->numbrushsides = c;
		n = LittleLong(in->textureindex);
		if (n < 0 || n >= loadmodel->num_textures)
...
static void Mod_Q3BSP_LoadEffects(lump_t *l)
{
	...
	loadmodel->t.brushq3.data_effects = out;
	loadmodel->t.brushq3.num_effects = count;

	for (i = 0;i < count;i++, in++, out++)
	{
		strlcpy (out->shadername, in->shadername, sizeof (out->shadername));
		n = LittleLong(in->t.brushindex);
		if (n >= loadmodel->t.brush.num_brushes)
		{
			Con_Printf("Mod_Q3BSP_LoadEffects: invalid brushindex %i (%i brushes), setting to -1\n", n, loadmodel->t.brush.num_brushes);
			n = -1;
		}
		out->t.brushindex = n;
...
static void Mod_Q3BSP_LoadVertices(lump_t *l)
{
	...
	loadmodel->t.brushq3.num_vertices = count = l->filelen / sizeof(*in);
	loadmodel->t.brushq3.data_vertex3f = (float *)Mem_Alloc(loadmodel->mempool, count * (sizeof(float) * (3 + 3 + 2 + 2 + 4)));
	loadmodel->t.brushq3.data_normal3f = loadmodel->t.brushq3.data_vertex3f + count * 3;
	loadmodel->t.brushq3.data_texcoordtexture2f = loadmodel->t.brushq3.data_normal3f + count * 3;
	loadmodel->t.brushq3.data_texcoordlightmap2f = loadmodel->t.brushq3.data_texcoordtexture2f + count * 2;
	loadmodel->t.brushq3.data_color4f = loadmodel->t.brushq3.data_texcoordlightmap2f + count * 2;

	for (i = 0;i < count;i++, in++)
	{
		loadmodel->t.brushq3.data_vertex3f[i * 3 + 0] = LittleFloat(in->origin3f[0]);
		loadmodel->t.brushq3.data_vertex3f[i * 3 + 1] = LittleFloat(in->origin3f[1]);
		loadmodel->t.brushq3.data_vertex3f[i * 3 + 2] = LittleFloat(in->origin3f[2]);
		loadmodel->t.brushq3.data_normal3f[i * 3 + 0] = LittleFloat(in->normal3f[0]);
		loadmodel->t.brushq3.data_normal3f[i * 3 + 1] = LittleFloat(in->normal3f[1]);
		loadmodel->t.brushq3.data_normal3f[i * 3 + 2] = LittleFloat(in->normal3f[2]);
		loadmodel->t.brushq3.data_texcoordtexture2f[i * 2 + 0] = LittleFloat(in->texcoord2f[0]);
		loadmodel->t.brushq3.data_texcoordtexture2f[i * 2 + 1] = LittleFloat(in->texcoord2f[1]);
		loadmodel->t.brushq3.data_texcoordlightmap2f[i * 2 + 0] = LittleFloat(in->lightmap2f[0]);
		loadmodel->t.brushq3.data_texcoordlightmap2f[i * 2 + 1] = LittleFloat(in->lightmap2f[1]);
		// svector/tvector are calculated later in face loading
		loadmodel->t.brushq3.data_color4f[i * 4 + 0] = in->color4ub[0] * (1.0f / 255.0f);
		loadmodel->t.brushq3.data_color4f[i * 4 + 1] = in->color4ub[1] * (1.0f / 255.0f);
		loadmodel->t.brushq3.data_color4f[i * 4 + 2] = in->color4ub[2] * (1.0f / 255.0f);
		loadmodel->t.brushq3.data_color4f[i * 4 + 3] = in->color4ub[3] * (1.0f / 255.0f);
	}
}
...
static void Mod_Q3BSP_LoadTriangles(lump_t *l)
{
	...
	loadmodel->t.brushq3.num_triangles = count / 3;
	loadmodel->t.brushq3.data_element3i = out;

	for (i = 0;i < count;i++, in++, out++)
	{
		*out = LittleLong(*in);
		if (*out < 0 || *out >= loadmodel->t.brushq3.num_vertices)
		{
			Con_Printf("Mod_Q3BSP_LoadTriangles: invalid vertexindex %i (%i vertices), setting to 0\n", *out, loadmodel->t.brushq3.num_vertices);
			*out = 0;
		}
	}
}
...
static void Mod_Q3BSP_LoadLightmaps(lump_t *l, lump_t *faceslump)
{
	...
	count = l->filelen / sizeof(*in);
	loadmodel->t.brushq3.num_originallightmaps = count;
	...
	loadmodel->t.brushq3.deluxemapping = !(count & 1);
	loadmodel->t.brushq3.deluxemapping_modelspace = true;
	endlightmap = 0;
	if (loadmodel->t.brushq3.deluxemapping)
	{
		...
		for (i = 0;i < facecount;i++)
		{
			...
			if (j >= 0)
			{
				...
				if ((j & 1) || j + 1 >= count)
				{
					loadmodel->t.brushq3.deluxemapping = false;
					break;
				}
			}
		}
	...
	Con_DPrintf("%s is %sdeluxemapped\n", loadmodel->name, loadmodel->t.brushq3.deluxemapping ? "" : "not ");

	// figure out what the most reasonable merge power is within limits
	loadmodel->t.brushq3.num_lightmapmergepower = 0;
	for (power = 1;power <= mod_q3bsp_lightmapmergepower.integer && (1 << power) <= gl_max_texture_size && (1 << (power * 2)) < 4 * (count >> loadmodel->t.brushq3.deluxemapping);power++)
		loadmodel->t.brushq3.num_lightmapmergepower = power;
	loadmodel->t.brushq3.num_lightmapmerge = 1 << loadmodel->t.brushq3.num_lightmapmergepower;

	loadmodel->t.brushq3.num_mergedlightmaps = ((count >> loadmodel->t.brushq3.deluxemapping) + (1 << (loadmodel->t.brushq3.num_lightmapmergepower * 2)) - 1) >> (loadmodel->t.brushq3.num_lightmapmergepower * 2);
	loadmodel->t.brushq3.data_lightmaps = (rtexture_t **)Mem_Alloc(loadmodel->mempool, loadmodel->t.brushq3.num_mergedlightmaps * sizeof(rtexture_t *));
	if (loadmodel->t.brushq3.deluxemapping)
		loadmodel->t.brushq3.data_deluxemaps = (rtexture_t **)Mem_Alloc(loadmodel->mempool, loadmodel->t.brushq3.num_mergedlightmaps * sizeof(rtexture_t *));

	j = 128 << loadmodel->t.brushq3.num_lightmapmergepower;
	if (loadmodel->t.brushq3.data_lightmaps)
		for (i = 0;i < loadmodel->t.brushq3.num_mergedlightmaps;i++)
			loadmodel->t.brushq3.data_lightmaps[i] = R_LoadTexture2D(loadmodel->texturepool, va("lightmap%04i", i), j, j, NULL, TEXTYPE_RGB, TEXF_FORCELINEAR | TEXF_PRECACHE, NULL);

	if (loadmodel->t.brushq3.data_deluxemaps)
		for (i = 0;i < loadmodel->t.brushq3.num_mergedlightmaps;i++)
			loadmodel->t.brushq3.data_deluxemaps[i] = R_LoadTexture2D(loadmodel->texturepool, va("deluxemap%04i", i), j, j, NULL, TEXTYPE_RGB, TEXF_FORCELINEAR | TEXF_PRECACHE, NULL);

	power = loadmodel->t.brushq3.num_lightmapmergepower;
	power2 = power * 2;
	mask = (1 << power) - 1;
	for (i = 0;i < count;i++)
	{
		j = i >> loadmodel->t.brushq3.deluxemapping;
		if (loadmodel->t.brushq3.deluxemapping && (i & 1))
			R_UpdateTexture(loadmodel->t.brushq3.data_deluxemaps[j >> power2], in[i].rgb, (j & mask) * 128, ((j >> power) & mask) * 128, 128, 128);
		else
			R_UpdateTexture(loadmodel->t.brushq3.data_lightmaps [j >> power2], in[i].rgb, (j & mask) * 128, ((j >> power) & mask) * 128, 128, 128);
	}
}
...
static void Mod_Q3BSP_LoadFaces(lump_t *l)
{
	...
	for (;i < count;i++, in++, out++)
	{
		...
		if (n < -1 || n >= loadmodel->t.brushq3.num_effects)
		{
#ifndef NO_DEVELOPER
			if (developer.integer >= 100)
				Con_Printf("Mod_Q3BSP_LoadFaces: face #%i (texture \"%s\"): invalid effectindex %i (%i effects)\n", i, out->texture->name, n, loadmodel->t.brushq3.num_effects);
#endif
			n = -1;
		}
		if (n == -1)
			...
			out->effect = loadmodel->t.brushq3.data_effects + n;
		...
		if (cls.state != ca_dedicated)
		{
			...
			if (n < 0)
				n = -1;
			else if (n >= loadmodel->t.brushq3.num_originallightmaps)
			{
				Con_Printf("Mod_Q3BSP_LoadFaces: face #%i (texture \"%s\"): invalid lightmapindex %i (%i lightmaps)\n", i, out->texture->name, n, loadmodel->t.brushq3.num_originallightmaps);
				n = -1;
			}
			else
			{
				out->lightmaptexture = loadmodel->t.brushq3.data_lightmaps[n >> (loadmodel->t.brushq3.num_lightmapmergepower * 2 + loadmodel->t.brushq3.deluxemapping)];
				if (loadmodel->t.brushq3.deluxemapping)
					out->deluxemaptexture = loadmodel->t.brushq3.data_deluxemaps[n >> (loadmodel->t.brushq3.num_lightmapmergepower * 2 + loadmodel->t.brushq3.deluxemapping)];
			}
		}
		...
		if (firstvertex < 0 || firstvertex + numvertices > loadmodel->t.brushq3.num_vertices)
		{
			Con_Printf("Mod_Q3BSP_LoadFaces: face #%i (texture \"%s\"): invalid vertex range %i : %i (%i vertices)\n", i, out->texture->name, firstvertex, firstvertex + numvertices, loadmodel->t.brushq3.num_vertices);
			continue;
		}
		if (firstelement < 0 || firstelement + numtriangles * 3 > loadmodel->t.brushq3.num_triangles * 3)
		{
			Con_Printf("Mod_Q3BSP_LoadFaces: face #%i (texture \"%s\"): invalid element range %i : %i (%i elements)\n", i, out->texture->name, firstelement, firstelement + numtriangles * 3, loadmodel->t.brushq3.num_triangles * 3);
			continue;
		}
		switch(type)
		{
		...
		case Q3FACETYPE_PATCH:
			...
			originalvertex3f = loadmodel->t.brushq3.data_vertex3f + firstvertex * 3;
	...
	for (;i < count && meshvertices + out->num_vertices <= loadmodel->surfmesh.num_vertices;i++, in++, out++)
	{
		...
		case Q3FACETYPE_MESH:
			...
			{
				(loadmodel->surfmesh.data_vertex3f + 3 * out->num_firstvertex)[j * 3 + 0] = loadmodel->t.brushq3.data_vertex3f[(firstvertex + j) * 3 + 0];
				(loadmodel->surfmesh.data_vertex3f + 3 * out->num_firstvertex)[j * 3 + 1] = loadmodel->t.brushq3.data_vertex3f[(firstvertex + j) * 3 + 1];
				(loadmodel->surfmesh.data_vertex3f + 3 * out->num_firstvertex)[j * 3 + 2] = loadmodel->t.brushq3.data_vertex3f[(firstvertex + j) * 3 + 2];
				(loadmodel->surfmesh.data_normal3f + 3 * out->num_firstvertex)[j * 3 + 0] = loadmodel->t.brushq3.data_normal3f[(firstvertex + j) * 3 + 0];
				(loadmodel->surfmesh.data_normal3f + 3 * out->num_firstvertex)[j * 3 + 1] = loadmodel->t.brushq3.data_normal3f[(firstvertex + j) * 3 + 1];
				(loadmodel->surfmesh.data_normal3f + 3 * out->num_firstvertex)[j * 3 + 2] = loadmodel->t.brushq3.data_normal3f[(firstvertex + j) * 3 + 2];
				(loadmodel->surfmesh.data_texcoordtexture2f + 2 * out->num_firstvertex)[j * 2 + 0] = loadmodel->t.brushq3.data_texcoordtexture2f[(firstvertex + j) * 2 + 0];
				(loadmodel->surfmesh.data_texcoordtexture2f + 2 * out->num_firstvertex)[j * 2 + 1] = loadmodel->t.brushq3.data_texcoordtexture2f[(firstvertex + j) * 2 + 1];
				(loadmodel->surfmesh.data_texcoordlightmap2f + 2 * out->num_firstvertex)[j * 2 + 0] = loadmodel->t.brushq3.data_texcoordlightmap2f[(firstvertex + j) * 2 + 0];
				(loadmodel->surfmesh.data_texcoordlightmap2f + 2 * out->num_firstvertex)[j * 2 + 1] = loadmodel->t.brushq3.data_texcoordlightmap2f[(firstvertex + j) * 2 + 1];
				(loadmodel->surfmesh.data_lightmapcolor4f + 4 * out->num_firstvertex)[j * 4 + 0] = loadmodel->t.brushq3.data_color4f[(firstvertex + j) * 4 + 0];
				(loadmodel->surfmesh.data_lightmapcolor4f + 4 * out->num_firstvertex)[j * 4 + 1] = loadmodel->t.brushq3.data_color4f[(firstvertex + j) * 4 + 1];
				(loadmodel->surfmesh.data_lightmapcolor4f + 4 * out->num_firstvertex)[j * 4 + 2] = loadmodel->t.brushq3.data_color4f[(firstvertex + j) * 4 + 2];
				(loadmodel->surfmesh.data_lightmapcolor4f + 4 * out->num_firstvertex)[j * 4 + 3] = loadmodel->t.brushq3.data_color4f[(firstvertex + j) * 4 + 3];
			}
			for (j = 0;j < out->num_triangles*3;j++)
				(loadmodel->surfmesh.data_element3i + 3 * out->num_firsttriangle)[j] = loadmodel->t.brushq3.data_element3i[firstelement + j] + out->num_firstvertex;
			break;
		case Q3FACETYPE_PATCH:
			patchsize[0] = LittleLong(in->specific.patch.patchsize[0]);
			patchsize[1] = LittleLong(in->specific.patch.patchsize[1]);
			originalvertex3f = loadmodel->t.brushq3.data_vertex3f + firstvertex * 3;
			originalnormal3f = loadmodel->t.brushq3.data_normal3f + firstvertex * 3;
			originaltexcoordtexture2f = loadmodel->t.brushq3.data_texcoordtexture2f + firstvertex * 2;
			originaltexcoordlightmap2f = loadmodel->t.brushq3.data_texcoordlightmap2f + firstvertex * 2;
			originalcolor4f = loadmodel->t.brushq3.data_color4f + firstvertex * 4;
			// convert patch to Q3FACETYPE_MESH
			xtess = Q3PatchTesselationOnX(patchsize[0], patchsize[1], 3, originalvertex3f, r_subdivisions_tolerance.value);
			ytess = Q3PatchTesselationOnY(patchsize[0], patchsize[1], 3, originalvertex3f, r_subdivisions_tolerance.value);
			// bound to user settings
		...
		if (out->num_vertices)
		{
			...
			{
				lightmapindex >>= loadmodel->t.brushq3.deluxemapping;
				lightmaptcscale = 1.0f / loadmodel->t.brushq3.num_lightmapmerge;
				lightmaptcbase[0] = ((lightmapindex                                             ) & (loadmodel->t.brushq3.num_lightmapmerge - 1)) * lightmaptcscale;
				lightmaptcbase[1] = ((lightmapindex >> loadmodel->t.brushq3.num_lightmapmergepower) & (loadmodel->t.brushq3.num_lightmapmerge - 1)) * lightmaptcscale;
	...
	Mod_BuildTextureVectorsFromNormals(0, loadmodel->surfmesh.num_vertices, loadmodel->surfmesh.num_triangles, loadmodel->surfmesh.data_vertex3f, loadmodel->surfmesh.data_texcoordtexture2f, loadmodel->surfmesh.data_normal3f, loadmodel->surfmesh.data_element3i,
#ifndef NO_ST_VECTORS
		       	loadmodel->surfmesh.data_svector3f, loadmodel->surfmesh.data_tvector3f,
#else
			NULL,NULL,
	...
	loadmodel->t.brushq3.num_vertices = 0;
	if (loadmodel->t.brushq3.data_vertex3f)
		Mem_Free(loadmodel->t.brushq3.data_vertex3f);
	loadmodel->t.brushq3.data_vertex3f = NULL;
	loadmodel->t.brushq3.data_normal3f = NULL;
	loadmodel->t.brushq3.data_texcoordtexture2f = NULL;
	loadmodel->t.brushq3.data_texcoordlightmap2f = NULL;
	loadmodel->t.brushq3.data_color4f = NULL;
	// free the no longer needed triangle data
	loadmodel->t.brushq3.num_triangles = 0;
	if (loadmodel->t.brushq3.data_element3i)
		Mem_Free(loadmodel->t.brushq3.data_element3i);
	loadmodel->t.brushq3.data_element3i = NULL;
}
...
static void Mod_Q3BSP_LoadModels(lump_t *l)
{
	...
	loadmodel->t.brushq3.data_models = out;
	loadmodel->t.brushq3.num_models = count;
	...
	for (i = 0;i < count;i++, in++, out++)
	{
		...
		if (n < 0 || n + c > loadmodel->t.brush.num_brushes)
			Host_Error("Mod_Q3BSP_LoadModels: invalid brush range %i : %i (%i brushes)", n, n + c, loadmodel->t.brush.num_brushes);
		out->firstbrush = n;
		out->numbrushes = c;
	}
}
...
static void Mod_Q3BSP_LoadLeafBrushes(lump_t *l)
{
	...
	loadmodel->t.brush.data_leafbrushes = out;
	loadmodel->t.brush.num_leafbrushes = count;

	for (i = 0;i < count;i++, in++, out++)
	{
		n = LittleLong(*in);
		if (n < 0 || n >= loadmodel->t.brush.num_brushes)
			Host_Error("Mod_Q3BSP_LoadLeafBrushes: invalid brush index %i (%i brushes)", n, loadmodel->t.brush.num_brushes);
		*out = n;
	}
}
...
static void Mod_Q3BSP_LoadLeafFaces(lump_t *l)
{
	...
	loadmodel->t.brush.data_leafsurfaces = out;
	loadmodel->t.brush.num_leafsurfaces = count;
...
static void Mod_Q3BSP_LoadLeafs(lump_t *l)
{
	...
	for (i = 0;i < count;i++, in++, out++)
	{
		...
		if (n < 0 || n + c > loadmodel->t.brush.num_leafsurfaces)
			Host_Error("Mod_Q3BSP_LoadLeafs: invalid leafsurface range %i : %i (%i leafsurfaces)", n, n + c, loadmodel->t.brush.num_leafsurfaces);
		out->firstleafsurface = loadmodel->t.brush.data_leafsurfaces + n;
		out->numleafsurfaces = c;
		n = LittleLong(in->firstleafbrush);
		c = LittleLong(in->numleafbrushes);
		if (n < 0 || n + c > loadmodel->t.brush.num_leafbrushes)
			Host_Error("Mod_Q3BSP_LoadLeafs: invalid leafbrush range %i : %i (%i leafbrushes)", n, n + c, loadmodel->t.brush.num_leafbrushes);
		out->firstleafbrush = loadmodel->t.brush.data_leafbrushes + n;
...
static void Mod_Q3BSP_LoadNodes(lump_t *l)
{
	...
	for (i = 0;i < count;i++, in++, out++)
	{
		...
		n = LittleLong(in->planeindex);
		if (n < 0 || n >= loadmodel->t.brush.num_planes)
			Host_Error("Mod_Q3BSP_LoadNodes: invalid planeindex %i (%i planes)", n, loadmodel->t.brush.num_planes);
		out->plane = loadmodel->t.brush.data_planes + n;
		for (j = 0;j < 2;j++)
		{
			...
			if (n >= 0)
			{
				if (n >= loadmodel->t.brush.num_nodes)
					Host_Error("Mod_Q3BSP_LoadNodes: invalid child node index %i (%i nodes)", n, loadmodel->t.brush.num_nodes);
				out->children[j] = loadmodel->t.brush.data_nodes + n;
			}
			else
			{
				n = -1 - n;
				if (n >= loadmodel->t.brush.num_leafs)
					Host_Error("Mod_Q3BSP_LoadNodes: invalid child leaf index %i (%i leafs)", n, loadmodel->t.brush.num_leafs);
				out->children[j] = (mnode_t *)(loadmodel->t.brush.data_leafs + n);
			}
		}
		for (j = 0;j < 3;j++)
		{
	...
	Mod_Q1BSP_LoadNodes_RecursiveSetParent(loadmodel->t.brush.data_nodes, NULL);
}
...
static void Mod_Q3BSP_LoadLightGrid(lump_t *l)
{
	...
	loadmodel->t.brushq3.num_lightgrid_scale[0] = 1.0f / loadmodel->t.brushq3.num_lightgrid_cellsize[0];
	loadmodel->t.brushq3.num_lightgrid_scale[1] = 1.0f / loadmodel->t.brushq3.num_lightgrid_cellsize[1];
	loadmodel->t.brushq3.num_lightgrid_scale[2] = 1.0f / loadmodel->t.brushq3.num_lightgrid_cellsize[2];
	loadmodel->t.brushq3.num_lightgrid_imins[0] = (int)dr_ceil(loadmodel->t.brushq3.data_models->mins[0] * loadmodel->t.brushq3.num_lightgrid_scale[0]);
	loadmodel->t.brushq3.num_lightgrid_imins[1] = (int)dr_ceil(loadmodel->t.brushq3.data_models->mins[1] * loadmodel->t.brushq3.num_lightgrid_scale[1]);
	loadmodel->t.brushq3.num_lightgrid_imins[2] = (int)dr_ceil(loadmodel->t.brushq3.data_models->mins[2] * loadmodel->t.brushq3.num_lightgrid_scale[2]);
	loadmodel->t.brushq3.num_lightgrid_imaxs[0] = (int)dr_floor(loadmodel->t.brushq3.data_models->maxs[0] * loadmodel->t.brushq3.num_lightgrid_scale[0]);
	loadmodel->t.brushq3.num_lightgrid_imaxs[1] = (int)dr_floor(loadmodel->t.brushq3.data_models->maxs[1] * loadmodel->t.brushq3.num_lightgrid_scale[1]);
	loadmodel->t.brushq3.num_lightgrid_imaxs[2] = (int)dr_floor(loadmodel->t.brushq3.data_models->maxs[2] * loadmodel->t.brushq3.num_lightgrid_scale[2]);
	loadmodel->t.brushq3.num_lightgrid_isize[0] = loadmodel->t.brushq3.num_lightgrid_imaxs[0] - loadmodel->t.brushq3.num_lightgrid_imins[0] + 1;
	loadmodel->t.brushq3.num_lightgrid_isize[1] = loadmodel->t.brushq3.num_lightgrid_imaxs[1] - loadmodel->t.brushq3.num_lightgrid_imins[1] + 1;
	loadmodel->t.brushq3.num_lightgrid_isize[2] = loadmodel->t.brushq3.num_lightgrid_imaxs[2] - loadmodel->t.brushq3.num_lightgrid_imins[2] + 1;
	count = loadmodel->t.brushq3.num_lightgrid_isize[0] * loadmodel->t.brushq3.num_lightgrid_isize[1] * loadmodel->t.brushq3.num_lightgrid_isize[2];
	Matrix4x4_CreateScale3(&loadmodel->t.brushq3.num_lightgrid_indexfromworld, loadmodel->t.brushq3.num_lightgrid_scale[0], loadmodel->t.brushq3.num_lightgrid_scale[1], loadmodel->t.brushq3.num_lightgrid_scale[2]);
	Matrix4x4_ConcatTranslate(&loadmodel->t.brushq3.num_lightgrid_indexfromworld, -loadmodel->t.brushq3.num_lightgrid_imins[0] * loadmodel->t.brushq3.num_lightgrid_cellsize[0], -loadmodel->t.brushq3.num_lightgrid_imins[1] * loadmodel->t.brushq3.num_lightgrid_cellsize[1], -loadmodel->t.brushq3.num_lightgrid_imins[2] * loadmodel->t.brushq3.num_lightgrid_cellsize[2]);

	// if lump is empty there is nothing to load, we can deal with that in the LightPoint code
	if (l->filelen)
	{
		if (l->filelen < count * (int)sizeof(*in))
			Host_Error("Mod_Q3BSP_LoadLightGrid: invalid lightgrid lump size %i bytes, should be %i bytes (%ix%ix%i)", l->filelen, (int)(count * sizeof(*in)), loadmodel->t.brushq3.num_lightgrid_dimensions[0], loadmodel->t.brushq3.num_lightgrid_dimensions[1], loadmodel->t.brushq3.num_lightgrid_dimensions[2]);
		if (l->filelen != count * (int)sizeof(*in))
			Con_Printf("Mod_Q3BSP_LoadLightGrid: Warning: calculated lightgrid size %i bytes does not match lump size %i\n", (int)(count * sizeof(*in)), l->filelen);
		out = (q3dlightgrid_t *)Mem_Alloc(loadmodel->mempool, count * sizeof(*out));
		loadmodel->t.brushq3.data_lightgrid = out;
		loadmodel->t.brushq3.num_lightgrid = count;
...
static void Mod_Q3BSP_LoadPVS(lump_t *l)
{
	...
	if (l->filelen == 0)
	{
		...
		loadmodel->t.brush.num_pvsclusters = 1;
		for (i = 0;i < loadmodel->t.brush.num_leafs;i++)
			loadmodel->t.brush.num_pvsclusters = max(loadmodel->t.brush.num_pvsclusters, loadmodel->t.brush.data_leafs[i].clusterindex + 1);

		// create clusters
		loadmodel->t.brush.num_pvsclusterbytes = (loadmodel->t.brush.num_pvsclusters + 7) / 8;
		totalchains = loadmodel->t.brush.num_pvsclusterbytes * loadmodel->t.brush.num_pvsclusters;
		loadmodel->t.brush.data_pvsclusters = (unsigned char *)Mem_Alloc(loadmodel->mempool, totalchains);
		memset(loadmodel->t.brush.data_pvsclusters, 0xFF, totalchains);
		return;
	}
	...
	loadmodel->t.brush.num_pvsclusters = LittleLong(in->numclusters);
	loadmodel->t.brush.num_pvsclusterbytes = LittleLong(in->chainlength);
	if (loadmodel->t.brush.num_pvsclusterbytes < ((loadmodel->t.brush.num_pvsclusters + 7) / 8))
		Host_Error("Mod_Q3BSP_LoadPVS: (chainlength = %i) < ((numclusters = %i) + 7) / 8", loadmodel->t.brush.num_pvsclusterbytes, loadmodel->t.brush.num_pvsclusters);
	totalchains = loadmodel->t.brush.num_pvsclusterbytes * loadmodel->t.brush.num_pvsclusters;
	if (l->filelen < totalchains + (int)sizeof(*in))
		Host_Error("Mod_Q3BSP_LoadPVS: lump too small ((numclusters = %i) * (chainlength = %i) + sizeof(q3dpvs_t) == %i bytes, lump is %i bytes)", loadmodel->t.brush.num_pvsclusters, loadmodel->t.brush.num_pvsclusterbytes, (int)(totalchains + sizeof(*in)), l->filelen);

	loadmodel->t.brush.data_pvsclusters = (unsigned char *)Mem_Alloc(loadmodel->mempool, totalchains);
	memcpy(loadmodel->t.brush.data_pvsclusters, (unsigned char *)(in + 1), totalchains);
}
...
static void Mod_Q3BSP_LightPoint(model_t *model, const vec3_t p, vec3_t ambientcolor, vec3_t diffusecolor, vec3_t diffusenormal)
{
	...
	if (!model->t.brushq3.num_lightgrid)
	{
	...
	Matrix4x4_Transform(&model->t.brushq3.num_lightgrid_indexfromworld, p, transformed);
	//Matrix4x4_Print(&model->t.brushq3.num_lightgrid_indexfromworld);
	//Con_Printf("%f %f %f transformed %f %f %f clamped ", p[0], p[1], p[2], transformed[0], transformed[1], transformed[2]);
	transformed[0] = bound(0, transformed[0], model->t.brushq3.num_lightgrid_isize[0] - 1);
	transformed[1] = bound(0, transformed[1], model->t.brushq3.num_lightgrid_isize[1] - 1);
	transformed[2] = bound(0, transformed[2], model->t.brushq3.num_lightgrid_isize[2] - 1);
	index[0] = (int)dr_floor(transformed[0]);
	index[1] = (int)dr_floor(transformed[1]);
	index[2] = (int)dr_floor(transformed[2]);
	//Con_Printf("%f %f %f index %i %i %i:\n", transformed[0], transformed[1], transformed[2], index[0], index[1], index[2]);

	// now lerp the values
	VectorClear(diffusenormal);
	a = &model->t.brushq3.data_lightgrid[(index[2] * model->t.brushq3.num_lightgrid_isize[1] + index[1]) * model->t.brushq3.num_lightgrid_isize[0] + index[0]];
	for (k = 0;k < 2;k++)
	{
		blend1 = (k ? (transformed[2] - index[2]) : (1 - (transformed[2] - index[2])));
		if (blend1 < 0.001f || index[2] + k >= model->t.brushq3.num_lightgrid_isize[2])
			continue;
		for (j = 0;j < 2;j++)
		{
			blend2 = blend1 * (j ? (transformed[1] - index[1]) : (1 - (transformed[1] - index[1])));
			if (blend2 < 0.001f || index[1] + j >= model->t.brushq3.num_lightgrid_isize[1])
				continue;
			for (i = 0;i < 2;i++)
			{
				blend = blend2 * (i ? (transformed[0] - index[0]) : (1 - (transformed[0] - index[0]))) * stylescale;
				if (blend < 0.001f || index[0] + i >= model->t.brushq3.num_lightgrid_isize[0])
					continue;
				s = a + (k * model->t.brushq3.num_lightgrid_isize[1] + j) * model->t.brushq3.num_lightgrid_isize[0] + i;
				...
				sinpitch = dr_sin(pitch);
				diffusenormal[0] += blend * (dr_cos(yaw) * sinpitch);
				diffusenormal[1] += blend * (dr_sin(yaw) * sinpitch);
				diffusenormal[2] += blend * (dr_cos(pitch));
				//Con_Printf("blend %f: ambient %i %i %i, diffuse %i %i %i, diffusepitch %i diffuseyaw %i (%f %f, normal %f %f %f)\n", blend, s->ambientrgb[0], s->ambientrgb[1], s->ambientrgb[2], s->diffusergb[0], s->diffusergb[1], s->diffusergb[2], s->diffusepitch, s->diffuseyaw, pitch, yaw, (dr_cos(yaw) * cospitch), (dr_sin(yaw) * cospitch), (-dr_sin(pitch)));
			}
		}
	}
...
static void Mod_Q3BSP_TracePoint_RecursiveBSPNode(trace_t *trace, model_t *model, mnode_t *node, const vec3_t point, int markframe)
{
	...
	{
		brush = model->t.brush.data_brushes[leaf->firstleafbrush[i]].colbrushf;
...
static void Mod_Q3BSP_TraceBox(model_t *model, int frame, trace_t *trace, const vec3_t start, const vec3_t boxmins, const vec3_t boxmaxs, const vec3_t end, int hitsupercontentsmask)
{
	...
	if (mod_q3bsp_optimizedtraceline.integer && VectorLength2(boxmins) + VectorLength2(boxmaxs) == 0)
	{
		...
		{
			// point trace
			if (model->t.brush.submodel)
			{
				for (i = 0, brush = model->t.brush.data_brushes + model->firstmodelbrush;i < model->nummodelbrushes;i++, brush++)
					if (brush->colbrushf)
						Collision_TracePointBrushFloat(trace, start, brush->colbrushf);
			}
			else
				Mod_Q3BSP_TracePoint_RecursiveBSPNode(trace, model, model->t.brush.data_nodes, start, ++markframe);
		}
		else
		{
			// line trace
			...
			if (model->t.brush.submodel)
			{
				...
				Mod_Q3BSP_TraceLine_RecursiveBSPNode(trace, model, model->t.brush.data_nodes, start, end, 0, 1, start, end, ++markframe, segmentmins, segmentmaxs);
		}
	}
	else
	{
		...
		if (model->t.brush.submodel)
		{
			...
			Mod_Q3BSP_TraceBrush_RecursiveBSPNode(trace, model, model->t.brush.data_nodes, thisbrush_start, thisbrush_end, ++markframe, segmentmins, segmentmaxs);
	}
}
...
void Mod_Q3BSP_RecursiveFindNumLeafs(mnode_t *node)
{
	...
	numleafs = ((mleaf_t *)node - loadmodel->t.brush.data_leafs) + 1;
	if (loadmodel->t.brush.num_leafs < numleafs)
		loadmodel->t.brush.num_leafs = numleafs;
}
...
void Mod_Q3BSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	mod->t.brush.TraceLineOfSight = Mod_Q1BSP_TraceLineOfSight;
	mod->t.brush.SuperContentsFromNativeContents = Mod_Q3BSP_SuperContentsFromNativeContents;
	mod->t.brush.NativeContentsFromSuperContents = Mod_Q3BSP_NativeContentsFromSuperContents;
	mod->t.brush.GetPVS = Mod_Q1BSP_GetPVS;
	mod->t.brush.FatPVS = Mod_Q1BSP_FatPVS;
	mod->t.brush.BoxTouchingPVS = Mod_Q1BSP_BoxTouchingPVS;
	mod->t.brush.BoxTouchingLeafPVS = Mod_Q1BSP_BoxTouchingLeafPVS;
	mod->t.brush.BoxTouchingVisibleLeafs = Mod_Q1BSP_BoxTouchingVisibleLeafs;
	mod->t.brush.FindBoxClusters = Mod_Q1BSP_FindBoxClusters;
	mod->t.brush.LightPoint = Mod_Q3BSP_LightPoint;
	mod->t.brush.FindNonSolidLocation = Mod_Q1BSP_FindNonSolidLocation;
	mod->t.brush.PointInLeaf = Mod_Q1BSP_PointInLeaf;
	mod->Draw = R_Q1BSP_Draw;
	mod->GetLightInfo = R_Q1BSP_GetLightInfo;
	...
	loadmodel->t.brush.numsubmodels = loadmodel->t.brushq3.num_models;

	// the MakePortals code works fine on the q3bsp data as well
	Mod_Q1BSP_MakePortals();

	// FIXME: shader alpha should replace r_wateralpha support in q3bsp
	loadmodel->t.brush.supportwateralpha = true;
	...
	loadmodel->t.brush.num_leafs = 0;
	Mod_Q3BSP_RecursiveFindNumLeafs(loadmodel->t.brush.data_nodes);
	...
	mod = loadmodel;
	for (i = 0;i < loadmodel->t.brush.numsubmodels;i++)
	{
		if (i > 0)
		{
			char name[10];
		...
		// make the model surface list (used by shadowing/lighting)
		mod->firstmodelsurface = mod->t.brushq3.data_models[i].firstface;
		mod->nummodelsurfaces = mod->t.brushq3.data_models[i].numfaces;
		mod->firstmodelbrush = mod->t.brushq3.data_models[i].firstbrush;
		mod->nummodelbrushes = mod->t.brushq3.data_models[i].numbrushes;
		mod->surfacelist = (int *)Mem_Alloc(loadmodel->mempool, mod->nummodelsurfaces * sizeof(*mod->surfacelist));
		for (j = 0;j < mod->nummodelsurfaces;j++)
			mod->surfacelist[j] = mod->firstmodelsurface + j;

		VectorCopy(mod->t.brushq3.data_models[i].mins, mod->normalmins);
		VectorCopy(mod->t.brushq3.data_models[i].maxs, mod->normalmaxs);
		corner[0] = max(dr_fabs(mod->normalmins[0]), dr_fabs(mod->normalmaxs[0]));
		corner[1] = max(dr_fabs(mod->normalmins[1]), dr_fabs(mod->normalmaxs[1]));
		corner[2] = max(dr_fabs(mod->normalmins[2]), dr_fabs(mod->normalmaxs[2]));
		modelradius = dr_sqrt(corner[0]*corner[0]+corner[1]*corner[1]+corner[2]*corner[2]);
		yawradius = dr_sqrt(corner[0]*corner[0]+corner[1]*corner[1]);
		mod->rotatedmins[0] = mod->rotatedmins[1] = mod->rotatedmins[2] = -modelradius;
		mod->rotatedmaxs[0] = mod->rotatedmaxs[1] = mod->rotatedmaxs[2] = modelradius;
...
void Mod_IBSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	Host_Error("Mod_IBSP_Load: Enabled NO_Q3SHADERS setting");
}
#endif
...
qboolean Mod_CanSeeBox_Trace(int numsamples, float t, model_t *model, vec3_t eye, vec3_t minsX, vec3_t maxsX)
{
	...
	if(model->t.brush.TraceLineOfSight(model, eye, testorigin))
		return 1;
