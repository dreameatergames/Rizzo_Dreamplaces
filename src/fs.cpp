...
#ifdef NO_ZLIB
#define qz_inflate(STR,FL) -6
#define qz_inflateEnd(STR) -6
#define qz_inflateInit2(STR,WB) -6
#define qz_inflateReset(STR) -6
#else
...
void FS_Rescan (void)
{
	...
	if (
#ifndef NO_GAMEMODE
			(gamemode == GAME_NORMAL || gamemode == GAME_HIPNOTIC || gamemode == GAME_ROGUE) &&
#endif
		       	!FS_FileExists("gfx/pop.lmp"))
	{
		if (fs_modified)
...
qfile_t *FS_OpenPackedFile (pack_t* pack, int pack_ind)
{
	...
	if (	
		...
		!zlib_dll &&
#endif
	       	(pfile->flags & PACKFILE_FLAG_DEFLATED))
	{
