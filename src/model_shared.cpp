...
model_t *Mod_LoadModel(model_t *mod, qboolean crash, qboolean checkdisk, qboolean isworldmodel)
{
	...
	if (buf)
	{
		...
		dbgf("Mod_LoadModel: New AllocPool for '%s'\n",mod->name);
...
{
	unsigned char *data;
	data = (unsigned char *)Mem_Alloc(mempool, numvertices * (3
#ifndef NO_ST_VECTORS
			       	+ 3 + 3
#endif
			       	+ 3 + 2 + 2 + (vertexcolors ? 4 : 0)) * sizeof(float) + numvertices * (lightmapoffsets ? 1 : 0) * sizeof(int) + numtriangles * (3 + (neighbors ? 3 : 0)) * sizeof(int));
...
#else
		size += maxverts * sizeof(float[11-3-3]);
#endif
...
#ifndef NO_ST_VECTORS
		 && (mesh->svector3f == NULL || (mesh->svector3f[vnum * 3 + 0] == vertex14f[3] && mesh->svector3f[vnum * 3 + 1] == vertex14f[4] && mesh->svector3f[vnum * 3 + 2] == vertex14f[5])) && (mesh->tvector3f == NULL || (mesh->tvector3f[vnum * 3 + 0] == vertex14f[6] && mesh->tvector3f[vnum * 3 + 1] == vertex14f[7] && mesh->tvector3f[vnum * 3 + 2] == vertex14f[8]))
#endif
...
void Mod_ShadowMesh_AddTriangle(mempool_t *mempool, shadowmesh_t *mesh, rtexture_t *map_diffuse, rtexture_t *map_specular, rtexture_t *map_normal, float *vertex14f)
{
	...
	{
		if (mesh->next == NULL)
			mesh->next = Mod_ShadowMesh_Alloc(mempool, max(mesh->maxverts, 300), max(mesh->maxtriangles, 100), map_diffuse, map_specular, map_normal,
#ifndef NO_ST_VECTORS
		       	mesh->svector3f != NULL,
#else
			0,
#endif
		       	mesh->neighbor3i != NULL, true);
		mesh = mesh->next;
	}
...
static void Mod_ShadowMesh_CreateVBOs(shadowmesh_t *mesh)
{
	...
	if (mesh->numverts)
	{
		...
		unsigned char *mem;
		size = 0;
		mesh->vbooffset_vertex3f = size;
		if (mesh->vertex3f)
		     	size += mesh->numverts * sizeof(float[3]);
#ifndef NO_ST_VECTORS
		mesh->vbooffset_svector3f = size;
		if (mesh->svector3f)
		       	size += mesh->numverts * sizeof(float[3]);
		mesh->vbooffset_tvector3f = size;
		if (mesh->tvector3f)
		       	size += mesh->numverts * sizeof(float[3]);
#endif
		mesh->vbooffset_normal3f= size;
		if (mesh->normal3f)
		       	size += mesh->numverts * sizeof(float[3]);
		mesh->vbooffset_texcoord2f = size;
		if (mesh->texcoord2f)
		       	size += mesh->numverts * sizeof(float[2]);
		mem = (unsigned char *)Mem_Alloc(tempmempool, size);
		if (mesh->vertex3f)
		       	memcpy(mem + mesh->vbooffset_vertex3f, mesh->vertex3f, mesh->numverts * sizeof(float[3]));
#ifndef NO_ST_VECTORS
...
void Mod_ConstructTerrainPatchFromRGBA(const unsigned char *imagepixels, int imagewidth, int imageheight, int x1, int y1, int width, int height, int *element3i, int *neighbor3i, float *vertex3f, float *svector3f, float *tvector3f, float *normal3f, float *texcoord2f, matrix4x4_t *pixelstepmatrix, matrix4x4_t *pixeltexturestepmatrix)
{
	...
	for (y = 0, iy = y1;y < height + 1;y++, iy++)
		for (x = 0, ix = x1;x < width + 1;x++, ix++, vertex3f += 3, texcoord2f += 2,
#ifndef NO_ST_VECTORS
...
void Mod_SnapVertices(int numcomponents, int numvertices, float *vertices, float snap)
{
	...
	for (i = 0;i < numvertices*numcomponents;i++)
		vertices[i] = dr_floor(vertices[i]*isnap)*snap;
}
