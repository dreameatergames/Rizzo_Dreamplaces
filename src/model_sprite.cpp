...
//
	...
	loadmodel->t.sprite.sprdata_frames = (mspriteframe_t *)Mem_Alloc(loadmodel->mempool, sizeof(mspriteframe_t) * realframes);
	...
	for (i = 0;i < loadmodel->numframes;i++)
	{
		...
		for (j = 0;j < groupframes;j++)
		{
			...
			loadmodel->t.sprite.sprdata_frames[realframes].left = origin[0];
			loadmodel->t.sprite.sprdata_frames[realframes].right = origin[0] + width;
			loadmodel->t.sprite.sprdata_frames[realframes].up = origin[1];
			loadmodel->t.sprite.sprdata_frames[realframes].down = origin[1] - height;

			x = (int)max(loadmodel->t.sprite.sprdata_frames[realframes].left * loadmodel->t.sprite.sprdata_frames[realframes].left, loadmodel->t.sprite.sprdata_frames[realframes].right * loadmodel->t.sprite.sprdata_frames[realframes].right);
			y = (int)max(loadmodel->t.sprite.sprdata_frames[realframes].up * loadmodel->t.sprite.sprdata_frames[realframes].up, loadmodel->t.sprite.sprdata_frames[realframes].down * loadmodel->t.sprite.sprdata_frames[realframes].down);
			if (modelradius < x + y)
				modelradius = x + y;
			...
			if (width > 0 && height > 0 && cls.state != ca_dedicated)
			{
				...
				if (!Mod_LoadSkinFrame(&loadmodel->t.sprite.sprdata_frames[realframes].texture.skinframes[0], name, (r_mipsprites.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_CLAMP | TEXF_PRECACHE | TEXF_PICMIP, false, false))
				{
					if (groupframes > 1)
					...
					if (version == SPRITE32_VERSION)
						Mod_LoadSkinFrame_Internal(&loadmodel->t.sprite.sprdata_frames[realframes].texture.skinframes[0], name, (r_mipsprites.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_CLAMP | TEXF_PRECACHE | TEXF_PICMIP, false, false, datapointer, width, height, 32, NULL, NULL);
					else //if (version == SPRITE_VERSION || version == SPRITEHL_VERSION)
						Mod_LoadSkinFrame_Internal(&loadmodel->t.sprite.sprdata_frames[realframes].texture.skinframes[0], name, (r_mipsprites.integer ? TEXF_MIPMAP : 0) | TEXF_ALPHA | TEXF_CLAMP | TEXF_PRECACHE | TEXF_PICMIP, false, false, datapointer, width, height, 8, palette, alphapalette);
				}
			}

			Mod_SpriteSetupTexture(&loadmodel->t.sprite.sprdata_frames[realframes], fullbright, additive);
...
void Mod_IDSP_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	if (version == SPRITE_VERSION || version == SPRITE32_VERSION)
	{
		...
		loadmodel->t.sprite.sprnum_type = LittleLong (pinqsprite->type);
	...
	else if (version == SPRITEHL_VERSION)
	{
		...
		loadmodel->t.sprite.sprnum_type = LittleLong (pinhlsprite->type);
...
void Mod_IDS2_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	loadmodel->t.sprite.sprnum_type = SPR_VP_PARALLEL;
	...
	loadmodel->animscenes = (animscene_t *)Mem_Alloc(loadmodel->mempool, sizeof(animscene_t) * loadmodel->numframes);
	loadmodel->t.sprite.sprdata_frames = (mspriteframe_t *)Mem_Alloc(loadmodel->mempool, sizeof(mspriteframe_t) * loadmodel->numframes);
	...
	for (i = 0;i < loadmodel->numframes;i++)
	{
		...
		sprframe = &loadmodel->t.sprite.sprdata_frames[i];
