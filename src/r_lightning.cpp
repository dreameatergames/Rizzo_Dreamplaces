...
#define PATHPOINTS 8
	...
	for (i = 0;i < 65536;i++)
	{
		...
		s = path[nearestpathindex].strength / dr_sqrt(dx*dx+dy*dy);
...
#define BEAMHEIGHT 128
	...
	for (y = 0;y < BEAMHEIGHT;y++)
	{
		...
		for (x = 0;x < BEAMWIDTH;x++, fx++)
		{
			...
			intensity = 1.0f - dr_sqrt(fx * fx);
			if (intensity > 0)
				intensity = dr_pow(intensity, 2) * ((noise2[y * BEAMHEIGHT + x] * (1.0f / 256.0f)) * 0.33f + 0.66f);
...
void R_DrawLightningBeam_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	for (surfacelistindex = 0;surfacelistindex < numsurfaces;surfacelistindex++)
	{
		...
		length = dr_sqrt(DotProduct(beamdir, beamdir));
