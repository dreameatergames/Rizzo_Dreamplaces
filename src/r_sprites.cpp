...
void R_Model_Sprite_Draw_TransparentCallback(const entity_render_t *ent, const rtlight_t *rtlight, int numsurfaces, int *surfacelist)
{
	...
	switch(model->t.sprite.sprnum_type)
	{
	case SPR_VP_PARALLEL_UPRIGHT:
		// flames and such
		// vertical beam sprite, faces view plane
		scale = ent->scale / dr_sqrt(r_view.forward[0]*r_view.forward[0]+r_view.forward[1]*r_view.forward[1]);
	...
	case SPR_FACING_UPRIGHT:
		...
		scale = ent->scale / dr_sqrt((org[0] - r_view.origin[0])*(org[0] - r_view.origin[0])+(org[1] - r_view.origin[1])*(org[1] - r_view.origin[1]));
	...
	for (i = 0;i < 4;i++)
	{
		...
		{
			mspriteframe_t *frame = model->t.sprite.sprdata_frames + ent->frameblend[i].frame;
			texture_t *texture = &frame->texture;
			R_UpdateTextureInfo(ent, texture);
			// FIXME: negate left and right in loader
			R_DrawSprite(texture->currentlayers[0].blendfunc1, texture->currentlayers[0].blendfunc2, frame->texture.currentskinframe->base,
				       	frame->texture.currentskinframe->fog,
				       	(texture->currentmaterialflags & MATERIALFLAG_NODEPTHTEST), (texture->currentmaterialflags & MATERIALFLAG_SHORTDEPTHRANGE), org, left, up, frame->left, frame->right, frame->down, frame->up, texture->currentlayers[0].color[0], texture->currentlayers[0].color[1], texture->currentlayers[0].color[2], ent->alpha * ent->frameblend[i].lerp);
		}
	}
}
