...
#if !defined(NO_NET) && !defined(DEBUG_NET)
#define netRand() rand()
#else
static int netRandnum=0;
#define netRand() (netRandnum++)
#endif
...
#undef Con_DPrint
#endif
#define Con_DPrint(T) dbg(T)
#ifdef Con_Print
#undef Con_Print
#endif
#define Con_Print(T) dbg(T)
#ifdef Con_Printf
#undef Con_Printf
#endif
#define Con_DPrint(T) dbg(T)
#define Con_Printf(FORMATO, RESTO...) dbgf(FORMATO , ## RESTO)
#ifndef NO_DEVELOPER
#define NO_DEVELOPER
#endif
#endif
...
#else
cvar_t sv_public = {-1, "sv_public", "-1", "Dummy Net"};
static cvar_t sv_heartbeatperiod = {99999, "sv_heartbeatperiod", "99999", "Dummy Net"};
#endif
...
int NetConn_Read(lhnetsocket_t *mysocket, void *data, int maxlength, lhnetaddress_t *peeraddress)
{
	...
	if (cl_netpacketloss.integer)
		for (i = 0;i < cl_numsockets;i++)
			if (cl_sockets[i] == mysocket && (netRand() % 100) < cl_netpacketloss.integer)
				return 0;
#ifndef NO_DEVELOPER
#ifndef DEBUG_NET
...
int NetConn_SendUnreliableMessage(netconn_t *conn, sizebuf_t *data, protocolversion_t protocol)
{
...
#ifdef DEBUG_NET
		dbg("NET: NetConn_SendUnreliableMessage PROTOCOL_QUAKEWORLD");
#endif
	...
	else
	{
		...
		{
#ifdef DEBUG_NET
			dbg("NET: NetConn_SendUnreliableMessage resend lost fragment");
#endif
		...
		{
#ifdef DEBUG_NET
			dbg("NET: NetConn_SendUnreliableMessage send new message");
#endif
			if (conn->message.cursize > (int)sizeof(conn->sendMessage))
			{
#ifdef DEBUG_NET
				dbgf("NET: NetConn_SendUnreliableMessage reliable message too big (%u > %u)\n",conn->message.cursize, (int)sizeof(conn->sendMessage));
#endif
		...
		{
#ifdef DEBUG_NET
			dbg("NET: NetConn_SendUnreliableMessage send a message");
#endif
			...
			{
#ifdef DEBUG_NET
				dbgf("NET: NetConn_SendUnreliableMessage: message too big %u\n", data->cursize);
#endif
			packetLen = NET_HEADERSIZE + data->cursize;
...
#ifdef DEBUG_NET
	dbgf("NET: NetConn_ReceivedMessage, len=%i\n",length);
#endif
...
static int NetConn_ClientParsePacket(lhnetsocket_t *mysocket, unsigned char *data, int length, lhnetaddress_t *peeraddress)
{
...
#ifdef DEBUG_NET
	dbg("NET: NetConn_ClientParsePacket");
#endif
	...
	{
		if (!cls.netcon)
		{	dbg("NET: NetConn_ClientParsePacket: !fromserver <- !cls.netcon"); }
		else
		if (cls.netcon->mysocket!= mysocket)
		{	dbgf("NET: NetConn_ClientParsePacket: !fromserver <- mysocket(%i) != cls.netcon->mysocket\n",mysocket,cls.netcon->mysocket); }
		else
		{	dbg("NET: NetConn_ClientParsePacket: !fromserver <- cls.netcon->peeraddress, peeraddress"); }
	}
#endif
	...
	if (length >= 5 && data[0] == 255 && data[1] == 255 && data[2] == 255 && data[3] == 255)
	{
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Added userinfo");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Darkplaces or Quake3");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Rejected");
#endif
			return true;
		}
		if (length >= 13 && !memcmp(string, "infoResponse\x0A", 13))
		{
			...
			{
#ifdef DEBUG_NET
				dbg("NET: NetConn_ClientParsePacket: InfoResponse < 0");
#endif
				return true;
			}
...
#ifndef NO_GAMEMODE
			if ((s = SearchInfostring(string, "gamename"     )) != NULL) strlcpy(info->game, s, sizeof (info->game));else
#endif
			       	info->game[0] = 0;
#ifndef NO_GAMEMODE
			if ((s = SearchInfostring(string, "modname"      )) != NULL) strlcpy(info->mod , s, sizeof (info->mod ));else
#endif
			       	info->mod[0]  = 0;
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: InfoResponse Updated");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: getserversResponsed");
#endif
			return true;
		}
		if (!memcmp(string, "d\n", 2) && serverlist_cachecount < SERVERLIST_TOTALSIZE)
		{
			...
			serverlist_querywaittime = realtime + 3;
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Begin or resume serverlist");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: extResponsed");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Received ping");
#endif
			return true;
		}
		...
		{
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: Ack");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: QuakeWorld compatibility updated");
#endif
			return true;
		}
...
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: QuakeWorld Accepted");
#endif
			return true;
		}
		if (length > 2 && !memcmp(string, "n\\", 2))
		{
			...
			{
#ifdef DEBUG_NET
				dbg("NET: NetConn_ClientParsePacket: QuakeWorld Status < 0");
#endif
				return true;
			}
			...
			NetConn_ClientParsePacket_ServerList_UpdateCache(n);
#ifdef DEBUG_NET
			dbg("NET: NetConn_ClientParsePacket: QuakeWorld Status Updated");
#endif
...
#ifdef DEBUG_NET
		dbg("NET: NetConn_ClientParsePacket: No liked packet");
#endif
		return true;
	}
#ifdef DEBUG_NET
	if (fromserver && cls.protocol == PROTOCOL_QUAKEWORLD && length >= 8)
	dbg("NET: NetConn_ClientParsePacket: quakeworld ingame packet");
#endif
	// quakeworld ingame packet
...
#ifdef DEBUG_NET
		dbg("NET: NetConn_ClientParsePacket: QuakeWorld ingame packet updated");
#endif
		return ret;
	}
	...
	if (length >= 5 && (control = BigLong(*((int *)data))) && (control & (~NETFLAG_LENGTH_MASK)) == (int)NETFLAG_CTL && (control & NETFLAG_LENGTH_MASK) == length)
	{
		...
		SZ_Clear(&net_message);
#ifdef DEBUG_NET
		dbg("NET: NetConn_ClientParsePacket: May not have liked the packed");
#endif
	...
	ret = 0;
#ifdef DEBUG_NET
	if (fromserver && length >= (int)NET_HEADERSIZE)
	{dbg("NET: NetConn_ClientParsePacket: CL_ParseServerMessage<-NetConn_ReceivedMessage");}
	else
	{dbgf("NET: NetConn_ClientParsePacket: !(fromserver(%i) && length(%i) >= NET_HEADERSIZE(%i))\n",fromserver,length,(int)NET_HEADERSIZE);}
#endif
...
#ifdef DEBUG_NET
	dbg("NET: NetConn_QueryQueueFrame");
#endif
...
#ifdef DEBUG_NET
	dbg("NET: NetConn_ClientFrame");
	if (cls.netcon)
		dbgf("NET: NetConn_ClientFrame: cls.netcon->address='%s', type %i\n",cls.netcon->address,cls.netcon->peeraddress.addresstype); 
#endif
	NetConn_UpdateSockets();
...
#ifndef NO_GAMEMODE
						gamename, com_modname, gameversion.integer,
#else
						"Dreamplaces", "Normal", "1.0",
					       	svs.maxclients,
#endif
...
#ifdef DEBUG_NET
	dbg("NET: NetConn_ServerParsePacket");
#endif
	// convert the address to a string incase we need it
	LHNETADDRESS_ToString(peeraddress, addressstring2, sizeof(addressstring2), true);
...
void NetConn_ServerFrame(void)
{
	...
	lhnetaddress_t peeraddress;
#ifdef DEBUG_NET
	dbg("NET: NetConn_ServerFrame");
	if (cls.netcon)
		dbgf("NET: NetConn_ServerFrame: cls.netcon->address='%s', type %i\n",cls.netcon->address,cls.netcon->peeraddress.addresstype); 
#endif
	for (i = 0;i < sv_numsockets;i++)
		...
		for (i = 0;i < cl_numsockets;i++)
		{
			if (cl_sockets[i])
			{
				...
				*((int *)net_message.data) = BigLong(NETFLAG_CTL | (net_message.cursize & NETFLAG_LENGTH_MASK));
				...
				dpsnprintf(request, sizeof(request), "\377\377\377\377getservers %s %u empty full\x0A",
#ifndef NO_GAMEMODE
					       	gamename,
#else
						"Dreamplaces",
