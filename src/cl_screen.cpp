...
static void SCR_CaptureVideo_RIFF_IndexEntry(const char *chunkfourcc, int chunksize, int flags)
{
	...
	if (cls.capturevideo.riffindexbuffer.cursize + 16 > cls.capturevideo.riffindexbuffer.maxsize)
	{
		...
		cls.capturevideo.riffindexbuffer.data = (unsigned char*)Mem_Alloc(tempmempool, cls.capturevideo.riffindexbuffer.maxsize);
		if (olddata)
		{
...
void SCR_CaptureVideo_BeginVideo(void)
{
	...
	for (i = 0;i < 256;i++)
	{
		unsigned char j = (unsigned char)bound(0, 255*dr_pow(i/255.0, gamma), 255);
...
*/
	...
	//else
	{
		...
		for (x = 1;x < 1000;x++)
			if (cls.capturevideo.framerate * x == dr_floor(cls.capturevideo.framerate * x))
				break;
...
void SCR_CaptureVideo(void)
{
	...
	if (cl_capturevideo.integer && r_render.integer)
	{
		...
		if (newframenum - cls.capturevideo.frame > (int)dr_ceil(cls.capturevideo.framerate))
		{
...
static void R_Envmap_f (void)
{
	...
	r_view.frustum_x = dr_tan(90 * M_PI / 360.0);
	r_view.frustum_y = dr_tan(90 * M_PI / 360.0);
...
qboolean SCR_ScreenShot(char *filename, unsigned char *buffer1, unsigned char *buffer2, unsigned char *buffer3, int x, int y, int width, int height, qboolean flipx, qboolean flipy, qboolean flipdiagonal, qboolean jpeg, qboolean gammacorrect)
{
	...
	if (scr_screenshot_gammaboost.value != 1 && gammacorrect)
	{
		...
		for (i = 0;i < 256;i++)
			ramp[i] = (unsigned char) (dr_pow(i * (1.0 / 255.0), igamma) * 255.0);
...
{
	dreamplaces_prof_start(PROFILER_BLIT);
	R_Mesh_Start();
	...
	if (cls.signon == SIGNONS)
	{
		...
		r_view.frustum_y = dr_tan(scr_fov.value * cl.viewzoom * M_PI / 360.0) * (3.0/4.0);
		...
		if (scr_zoomwindow.integer)
		{
			...
			r_view.frustum_y = dr_tan(scr_zoomwindow_fov.value * cl.viewzoom * M_PI / 360.0) * (3.0/4.0);
...
#endif
	dreamplaces_prof_end(PROFILER_BLIT);
}
