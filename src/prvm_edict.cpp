...
char *PRVM_UglyValueString (etype_t type, prvm_eval_t *val)
{
	...
	case ev_void:
		...
		break;
	case ev_float:
		dpsnprintf (line, sizeof (line), "%f", (double)val->_float);
		break;
	case ev_vector:
		dpsnprintf (line, sizeof (line), "%f %f %f", (double)val->vector[0], (double)val->vector[1], (double)val->vector[2]);
		break;
	default:
