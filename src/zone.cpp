...
#ifdef MEM_STATS
#define MEMSTATS_MAX_ALLOCS (128*1024)
#include<stdio.h>
static unsigned memstats_max=0;
static unsigned memstats_actual=0;
static void *memstats_allocs[MEMSTATS_MAX_ALLOCS];
static unsigned memstats_sizes[MEMSTATS_MAX_ALLOCS];
static unsigned memstats_n_allocs=0;

static void memstats_alloc(const char *msg, unsigned len, void *p)
{
	if (memstats_actual+len>memstats_max)
		memstats_max=memstats_actual+len;
	printf("MEM ALLOC: %X + %X = %X (%X max) -> %p <- %s\n",memstats_actual,len,memstats_actual+len,memstats_max,p,msg); fflush(stdout);
	memstats_actual+=len;
	if (memstats_n_allocs>MEMSTATS_MAX_ALLOCS)
	{
		printf("ERROR: MEM ALLOCS > %i\n",MEMSTATS_MAX_ALLOCS);
		fflush(stdout);
		exit(0);
	}
	memstats_allocs[memstats_n_allocs]=p;
	memstats_sizes[memstats_n_allocs++]=len;
}

static void memstats_free(const char *msg, void *p)
{
	int i;
	for(i=0;i<memstats_n_allocs;i++)
		if (memstats_allocs[i]==p)
			break;
	if (i>=memstats_n_allocs)
	{
		printf("ERROR: FREE %p NOT FOUND\n",p);
		fflush(stdout);
		exit(0);
	}
	printf("MEM FREE: %X - %X = %X (%X max) -> %p <- %s\n",memstats_actual,memstats_sizes[i],memstats_actual-memstats_sizes[i],memstats_max,memstats_allocs[i],msg);
	fflush(stdout);
	memstats_actual-=memstats_sizes[i];
	memstats_allocs[i]=NULL;
}
#else
#define memstats_alloc(MSG,LEN,P)	
#define memstats_free(MSG,P)	
#endif
...
void *_Mem_Alloc(mempool_t *pool, size_t size, const char *filename, int fileline)
{
	...
	loopcontinue:;
		...
		memstats_alloc("Mem_Alloc clump",sizeof(memclump_t),clump);
		if (clump == NULL)
...
choseclump:
	...
	else
	{
		...
		memstats_alloc("Mem_Alloc",sizeof(memheader_t) + size + sizeof(int),(void *)mem);
		if (mem == NULL)
			Sys_Error("Mem_Alloc: out of memory (alloc at %s:%i)", filename, fileline);
#if MEMCLUMPING
		mem->clump = NULL;
...
static void _Mem_FreeBlock(memheader_t *mem, const char *filename, int fileline)
{
	...
	if ((clump = mem->clump))
	{
		...
		if (clump->blocksinuse <= 0)
		{
			...
			free(clump);
			memstats_free("Mem_FreeBlock Clump",(void *)clump);
		}
		else
		{
...
#endif
		free(mem);
		memstats_free("Mem_FreeBlock",(void *)mem);
#if MEMCLUMPING
	}
#endif
}
...
mempool_t *_Mem_AllocPool(const char *name, int flags, mempool_t *parent, const char *filename, int fileline)
{
	...
	memstats_alloc("Mem_AllocPool",sizeof(mempool_t),(void *)pool);
	if (pool == NULL)
...
void _Mem_FreePool(mempool_t **poolpointer, const char *filename, int fileline)
{
	...
	if (pool)
	{
		...
		*chainaddress = pool->next;
		...
		free(pool);
		memstats_free("Mem_FreePool",(void *)pool);
