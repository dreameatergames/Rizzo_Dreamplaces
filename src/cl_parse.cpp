...
void CL_ParseEntityLump(char *entdata)
{
	...
	R_SetSkyBox(cl.worldmodel->t.brush.skybox);
	data = entdata;
	if (!data)
		return;
...
static void QW_CL_RequestNextDownload(void)
{
	...
	case dl_model:
		...
		MSG_WriteByte(&cls.netcon->message, qw_clc_stringcmd);
		MSG_WriteString(&cls.netcon->message, va("prespawn %i 0 %i", cl.qw_servercount, cl.model_precache[1]->t.brush.qw_md4sum2));
...
void CL_ParseDownload(void)
{
	...
	cls.qw_downloadpercent = (int)dr_floor((start+size) * 100.0 / cls.qw_downloadmemorymaxsize);
...
{
	if ((cl.islocalgame && !sv_fixedframeratesingleplayer.integer) || cl.mtime[0] == newtime || cls.signon < SIGNONS
#ifndef NO_DEMO
		|| cls.timedemo 
#endif
	   )
