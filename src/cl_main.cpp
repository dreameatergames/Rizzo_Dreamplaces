...
#define MAX_PARTICLES		(32768 / REDUCE_DIVISOR_PARTICLES)	// default max # of particles at one time
#define ABSOLUTE_MIN_PARTICLES	(512 / REDUCE_DIVISOR_PARTICLES)		// no fewer than this no matter what's on the command line
...
// wipe the entire cl structure
	...
	cl.max_entities = 256 / REDUCE_DIVISOR_ENTITIES;
	cl.max_static_entities = 256 / REDUCE_DIVISOR_ENTITIES;
	cl.max_temp_entities = 512 / REDUCE_DIVISOR_ENTITIES;
	cl.max_effects = 256 / REDUCE_DIVISOR_EFFECTS;
	cl.max_beams = 256 / REDUCE_DIVISOR_BEAMS;
	...
	if (e->render.model)
	{
		...
		if ((e->render.model->flags & EF_ROTATE) && (!e->state_current.tagentity && !(e->render.flags & RENDER_VIEWMODEL)))
		{
			...
			if (cl_itembobheight.value)
				origin[2] += (dr_cos(cl.time * cl_itembobspeed.value * (2.0 * M_PI)) + 1.0) * 0.5 * cl_itembobheight.value;
		}
...
void CL_RelinkBeams(void)
{
	...
	for (i = 0, b = cl.beams;i < cl.num_beams;i++, b++)
	{
		...
		{
			yaw = (int) (dr_atan2(dist[1], dist[0]) * 180 / M_PI);
			if (yaw < 0)
				yaw += 360;

			forward = dr_sqrt (dist[0]*dist[0] + dist[1]*dist[1]);
			pitch = (int) (dr_atan2(dist[2], forward) * 180 / M_PI);
			if (pitch < 0)
				pitch += 360;
		}
...
void CL_Locs_FindLocationName(char *buffer, size_t buffersize, vec3_t point)
{
	...
	if (loc)
		...
		dpsnprintf(buffer, buffersize, "LOC=%.0f:%.0f:%.0f", (double)point[0], (double)point[1], (double)point[2]);
}
...
void CL_Locs_Save_f(void)
{
	...
	for (loc = cl.locnodes;loc;loc = loc->next)
	{
		if (VectorCompare(loc->mins, loc->maxs))
		{
			...
			name[len] = 0;
			FS_Printf(outfile, "%.0f %.0f %.0f %s\n", (double)loc->mins[0]*8, (double)loc->mins[1]*8, (double)loc->mins[2]*8, name);
		}
		else
			FS_Printf(outfile, "%.1f,%.1f,%.1f,%.1f,%.1f,%.1f,\"%s\"\n", (double)loc->mins[0], (double)loc->mins[1], (double)loc->mins[2], (double)loc->maxs[0], (double)loc->maxs[1], (double)loc->maxs[2], loc->name);
	}
	FS_Close(outfile);
}
