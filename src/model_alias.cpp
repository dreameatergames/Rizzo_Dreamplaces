...
void Mod_AliasInit (void)
{
	...
	for (i = 0;i < 320;i++)
		mod_md3_sin[i] = dr_sin(i * M_PI * 2.0f / 256.0);
}
...
static void Mod_Alias_CalculateBoundingBox(void)
{
	...
	Mem_Free(vertex3f);
	radius = dr_sqrt(radius);
	yawradius = dr_sqrt(yawradius);
...
static void Mod_Alias_Mesh_CompileFrameZero(void)
{
	...
	loadmodel->surfmesh.data_vertex3f = (float *)Mem_Alloc(loadmodel->mempool, loadmodel->surfmesh.num_vertices *
#ifndef NO_ST_VECTORS
		       	sizeof(float[3][4])
#else
		       	sizeof(float[3][4-2])
...
#else
	loadmodel->surfmesh.data_normal3f = loadmodel->surfmesh.data_vertex3f + loadmodel->surfmesh.num_vertices * 3;
#endif
	Mod_Alias_GetMesh_Vertices(loadmodel, frameblend, loadmodel->surfmesh.data_vertex3f, loadmodel->surfmesh.data_normal3f,
#ifndef NO_ST_VECTORS
...
void Mod_ZYMOTICMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	corner[0] = max(dr_fabs(loadmodel->normalmins[0]), dr_fabs(loadmodel->normalmaxs[0]));
	corner[1] = max(dr_fabs(loadmodel->normalmins[1]), dr_fabs(loadmodel->normalmaxs[1]));
	loadmodel->yawmaxs[0] = loadmodel->yawmaxs[1] = dr_sqrt(corner[0]*corner[0]+corner[1]*corner[1]);
	...
	loadmodel->nummodelsurfaces = loadmodel->num_surfaces;
	loadmodel->num_textures = loadmodel->num_surfaces;
	data = (unsigned char *)Mem_Alloc(loadmodel->mempool, loadmodel->num_surfaces * sizeof(msurface_t) + loadmodel->num_surfaces * sizeof(int) + loadmodel->num_surfaces * loadmodel->numskins * sizeof(texture_t) + meshtriangles * sizeof(int[3]) + meshtriangles * sizeof(int[3]) +
#ifndef NO_ST_VECTORS
		       	meshvertices * sizeof(float[14]) +
#else
			meshvertices * sizeof(float[14-3-3]) +
#endif
		       	meshvertices * sizeof(int[4]) + meshvertices * sizeof(float[4]) + loadmodel->num_poses * sizeof(float[36]));
	loadmodel->data_surfaces = (msurface_t *)data;data += loadmodel->num_surfaces * sizeof(msurface_t);
	loadmodel->surfacelist = (int *)data;data += loadmodel->num_surfaces * sizeof(int);
	loadmodel->data_textures = (texture_t *)data;data += loadmodel->num_surfaces * loadmodel->numskins * sizeof(texture_t);
	loadmodel->surfmesh.num_vertices = meshvertices;
...
void Mod_DARKPLACESMODEL_Load(model_t *mod, void *buffer, void *bufferend)
{
	...
	data = (unsigned char *)Mem_Alloc(loadmodel->mempool, loadmodel->num_surfaces * sizeof(msurface_t) + loadmodel->num_surfaces * sizeof(int) + loadmodel->num_surfaces * loadmodel->numskins * sizeof(texture_t) + meshtriangles * sizeof(int[3]) + meshtriangles * sizeof(int[3]) +
		       	...
		       	meshvertices * (sizeof(float[14-3-3]) +
#endif
			sizeof(int[4]) + sizeof(float[4])) + loadmodel->num_poses * sizeof(float[36]) + loadmodel->numskins * sizeof(animscene_t) + loadmodel->num_bones * sizeof(aliasbone_t) + loadmodel->numframes * sizeof(animscene_t));
	loadmodel->data_surfaces = (msurface_t *)data;data += loadmodel->num_surfaces * sizeof(msurface_t);
	loadmodel->surfacelist = (int *)data;data += loadmodel->num_surfaces * sizeof(int);
	loadmodel->data_textures = (texture_t *)data;data += loadmodel->num_surfaces * loadmodel->numskins * sizeof(texture_t);
	loadmodel->surfmesh.num_vertices = meshvertices;
	...
	for (i = 0;i < loadmodel->num_surfaces;i++, dpmmesh++)
	{
		...
		for (j = surface->num_firstvertex;j < surface->num_firstvertex + surface->num_vertices;j++)
		{
			...
			if (sum && dr_fabs(sum - 1) > (1.0f / 256.0f))
			{
				float f = 1.0f / sum;
				for (l = 0;l < 4;l++)
