...
#ifndef NO_GLEXTENSIONS
static unsigned int backendunits, backendimageunits, backendarrayunits;
#else
#define backendunits 1
#define backendimageunits backendunits
#define backendarrayunits backendunits
#endif
...
void GL_SetupView_Mode_Perspective (dr_real_t frustumx, dr_real_t frustumy, dr_real_t zNear, dr_real_t zFar)
{
	dr_real_t m[16];
...
void GL_SetupView_Mode_PerspectiveInfiniteFarClip (dr_real_t frustumx, dr_real_t frustumy, dr_real_t zNear)
{
	dr_real_t nudge, m[16];
...
void GL_SetupView_Mode_Ortho (dr_real_t x1, dr_real_t y1, dr_real_t x2, dr_real_t y2, dr_real_t zNear, dr_real_t zFar)
{
	dr_real_t m[16];
...
void R_Mesh_Draw(int firstvertex, int numvertices, int numtriangles, const int *elements, int bufferobject, size_t bufferoffset)
{
	...
	if (!gl_vbo.integer)
		bufferobject = 0;
#endif
	dreamplaces_prof_start(PROFILER_R0);
	CHECKGLERROR
...
#endif
		{
			dreamplaces_prof_start(PROFILER_R1);
#ifndef NO_GLEXTENSIONS
			GL_BindEBO(bufferobject);
#endif
			qglDrawElements(GL_TRIANGLES, numelements, GL_UNSIGNED_INT, bufferobject ? (void *)bufferoffset : elements);
			CHECKGLERROR
			dreamplaces_prof_end(PROFILER_R1);
		}
	}
	dreamplaces_prof_end(PROFILER_R0);
}
