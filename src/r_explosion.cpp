...
static int R_ExplosionVert(int column, int row)
{
	...
	yaw = ((dr_real_t) column / EXPLOSIONGRID) * M_PI * 2;
	pitch = (((dr_real_t) row / EXPLOSIONGRID) - 0.5) * M_PI;
	explosionpoint[i][0] = dr_cos(yaw) *  dr_cos(pitch);
	explosionpoint[i][1] = dr_sin(yaw) *  dr_cos(pitch);
	explosionpoint[i][2] =        1 * - dr_sin(pitch);
