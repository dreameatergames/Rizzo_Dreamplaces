...
void R_SkyStartFrame(void)
{
 	   ...
 	   )
	{
		if (skyboxside[0] || skyboxside[1] || skyboxside[2] || skyboxside[3] || skyboxside[4] || skyboxside[5])
			skyrenderbox = true;
		else if (r_refdef.worldmodel && r_refdef.worldmodel->t.brush.solidskytexture)
...
static void skyspherecalc(void)
{
	...
	for (j = 0;j <= skygridy;j++)
	{
		...
		for (i = 0;i <= skygridx;i++)
		{
			...
			v[0] = ax*x * dx;
			v[1] = ay*x * dy;
			v[2] = - dr_sin((b + 0.5) * M_PI) * dz;
			length = 3.0f / dr_sqrt(v[0]*v[0]+v[1]*v[1]+(v[2]*v[2]*9));
...
static void R_SkySphere(void)
{
	...
	R_Mesh_ColorPointer(NULL, 0, 0);
	R_Mesh_ResetTextureState();
	R_Mesh_TexBind(0, R_GetTexture(r_refdef.worldmodel->t.brush.solidskytexture));
	...
	if (r_textureunits.integer >= 2 && r_view.colorscale == 1)
	{
		...
		R_Mesh_TexBind(1, R_GetTexture(r_refdef.worldmodel->t.brush.alphaskytexture));
...
#endif
	{
		...
		R_Mesh_TexBind(0, R_GetTexture(r_refdef.worldmodel->t.brush.alphaskytexture));
