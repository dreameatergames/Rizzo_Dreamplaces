	...
	if (src[0])
	{
		...
		if (src[1])
		{
			...
			if (src[2])
			{
				dst[2] = 0;
				pos = 0;
				minelem = dr_fabs(src[0]);
				if (dr_fabs(src[1]) < minelem)
				{
					pos = 1;
					minelem = dr_fabs(src[1]);
				}
				if (dr_fabs(src[2]) < minelem)
					pos = 2;
...
float RadiusFromBounds (const vec3_t mins, const vec3_t maxs)
{
	...
	return dr_sqrt(max(m1[0], m2[0]) + max(m1[1], m2[1]) + max(m1[2], m2[2]));
}
