...
dr_real_t bsplinesample(int dimensions, dr_real_t t, dr_real_t *param)
{
	dr_real_t o = 0;
	for (i = 0;i < dimensions + 1;i++)
		o += param[i] * factorial(dimensions)/(factorial(i)*factorial(dimensions-i)) * dr_pow(t, i) * dr_pow(1 - t, dimensions - i);
	return o;
}
*/
...
int Q3PatchTesselationOnX(int patchwidth, int patchheight, int components, const float *in, float tolerance)
{
	...
	return (int)dr_floor(dr_log(dr_sqrt(bestsquareddeviation) / tolerance) / dr_log(2)) + 1;
}
