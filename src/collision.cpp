	...
	if (brush->numplanes)
	{
		...
		for (j = 0;j < brush->numpoints;j++)
		{
			...
			for (k = 0;k < brush->numplanes;k++)
			{
				...
				if (dr_fabs(d) > COLLISION_PLANE_DIST_EPSILON)
					pointsoffplanes++;
				else
					pointonplanes++;
			}
...
colbrushf_t *Collision_NewBrushFromPlanes(mempool_t *mempool, int numoriginalplanes, const colplanef_t *originalplanes, int supercontents)
{
	...
	for (j = 0;j < numoriginalplanes;j++)
	{
		...
		for (k = 0;k < pnumpoints;k++)
		{
			...
			for (l = 0;l < numoriginalplanes;l++)
				if (dr_fabs(DotProduct(&p[w][k*3], originalplanes[l].normal) - originalplanes[l].dist) < COLLISION_PLANE_DIST_EPSILON)
					m++;
			if (m < 3)
				break;
		}
	...
	for (j = 0;j < numplanesbuf;j++)
	{
		...
		if (dr_fabs(planesbuf[j].dist - d) > COLLISION_PLANE_DIST_EPSILON)
	...
	if (brush->numpoints == 3)
	{
		...
		else
		{
...
#if 1
			{
				...
				int i, best;
				float dist, bestdist;
				bestdist = dr_fabs(brush->planes[0].normal[0]);
				best = 0;
				for (i = 1;i < 3;i++)
				{
					dist = dr_fabs(brush->planes[0].normal[i]);
					if (bestdist < dist)
					{
						bestdist = dist;
						best = i;
			...
			if (developer.integer >= 100)
			{
				...
				if (dr_fabs(DotProduct(brush->planes[1].normal, brush->planes[0].normal) - -1.0f) > 0.01f || dr_fabs(brush->planes[1].dist - -brush->planes[0].dist) > 0.01f)
...
#if 0
				if (dr_fabs(DotProduct(brush->planes[2].normal, brush->planes[0].normal)) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->planes[3].normal, brush->planes[0].normal)) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->planes[4].normal, brush->planes[0].normal)) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->planes[2].normal, edge0)) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->planes[3].normal, edge1)) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->planes[4].normal, edge2)) > 0.01f)
...
#endif
#endif
				if (dr_fabs(DotProduct(brush->points[0].v, brush->planes[0].normal) - brush->planes[0].dist) > 0.01f || dr_fabs(DotProduct(brush->points[1].v, brush->planes[0].normal) - brush->planes[0].dist) > 0.01f || dr_fabs(DotProduct(brush->points[2].v, brush->planes[0].normal) - brush->planes[0].dist) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->points[0].v, brush->planes[1].normal) - brush->planes[1].dist) > 0.01f || dr_fabs(DotProduct(brush->points[1].v, brush->planes[1].normal) - brush->planes[1].dist) > 0.01f || dr_fabs(DotProduct(brush->points[2].v, brush->planes[1].normal) - brush->planes[1].dist) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->points[2].v, brush->planes[2].normal) - brush->planes[2].dist) > 0.01f || dr_fabs(DotProduct(brush->points[0].v, brush->planes[2].normal) - brush->planes[2].dist) > 0.01f)
				...
				if (dr_fabs(DotProduct(brush->points[0].v, brush->planes[3].normal) - brush->planes[3].dist) > 0.01f || dr_fabs(DotProduct(brush->points[1].v, brush->planes[3].normal) - brush->planes[3].dist) > 0.01f)
					Con_Printf("Collision_CalcPlanesForPolygonBrushFloat: edge 0 (%f %f %f to %f %f %f) off front plane 2 (%f %f %f %f)\n", brush->points[0].v[0], brush->points[0].v[1], brush->points[0].v[2], brush->points[1].v[0], brush->points[1].v[1], brush->points[1].v[2], brush->planes[3].normal[0], brush->planes[3].normal[1], brush->planes[3].normal[2], brush->planes[3].dist);
				if (dr_fabs(DotProduct(brush->points[1].v, brush->planes[4].normal) - brush->planes[4].dist) > 0.01f || dr_fabs(DotProduct(brush->points[2].v, brush->planes[4].normal) - brush->planes[4].dist) > 0.01f)
	...
	for (nplane = 0;nplane < thatbrush_start->numplanes + thisbrush_start->numplanes;nplane++)
	{
		...
		if (nplane2 >= thatbrush_start->numplanes)
		{
			...
			if (developer.integer >= 100)
			{
				...
				if (dr_fabs(f - startplane->dist) > COLLISION_PLANE_DIST_EPSILON)
...
void Collision_SnapCopyPoints(int numpoints, const colpointf_t *in, colpointf_t *out, float fractionprecision, float invfractionprecision)
{
	...
	{
		out[i].v[0] = dr_floor(in[i].v[0] * fractionprecision + 0.5f) * invfractionprecision;
		out[i].v[1] = dr_floor(in[i].v[1] * fractionprecision + 0.5f) * invfractionprecision;
		out[i].v[2] = dr_floor(in[i].v[2] * fractionprecision + 0.5f) * invfractionprecision;
	}
}
...
float Collision_ClipTrace_Line_Sphere(dr_real_t *linestart, dr_real_t *lineend, dr_real_t *sphereorigin, dr_real_t sphereradius, dr_real_t *impactpoint, dr_real_t *impactnormal)
{
	dr_real_t dir[3], scale, v[3], deviationdist, impactdist, linelength;
...
void Collision_TraceLineTriangleFloat(trace_t *trace, const vec3_t linestart, const vec3_t lineend, const float *point0, const float *point1, const float *point2, int supercontents, int q3surfaceflags, texture_t *texture)
{
	...
	d = 1.0 / dr_sqrt(faceplanenormallength2);
