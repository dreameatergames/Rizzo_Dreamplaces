...
#endif
{
	...
	if (sv.worldmodel && sv.worldmodel->t.brush.RoundUpToHullSize)
		sv.worldmodel->t.brush.RoundUpToHullSize(sv.worldmodel, clipmins, clipmaxs, hullmins, hullmaxs);
	else
	{
...
//
	...
	{
		wishspeed = sv_maxvelocity.value / dr_sqrt(wishspeed);
...
void SV_PushMove (prvm_edict_t *pusher, float movetime)
{
	...
	case SOLID_TRIGGER:
		...
		pusher->fields.server->angles[0] -= 360.0 * dr_floor(pusher->fields.server->angles[0] * (1.0 / 360.0));
		pusher->fields.server->angles[1] -= 360.0 * dr_floor(pusher->fields.server->angles[1] * (1.0 / 360.0));
		pusher->fields.server->angles[2] -= 360.0 * dr_floor(pusher->fields.server->angles[2] * (1.0 / 360.0));
		pusher->fields.server->ltime += movetime;
...
// see if any solid entities are inside the final position
	...
	for (e = 0;e < numcheckentities;e++)
	{
		...
		{
			Collision_ClipToGenericEntity(&trace, pushermodel, (int)pusher->fields.server->frame, pusher->fields.server->mins, pusher->fields.server->maxs, SUPERCONTENTS_BODY, &pusherfinalmatrix, &pusherfinalimatrix, check->fields.server->origin, check->fields.server->mins, check->fields.server->maxs, check->fields.server->origin, SUPERCONTENTS_SOLID | SUPERCONTENTS_BODY);
			if (!trace.startsolid)
				continue;
		}
...
int SV_TryUnstick (prvm_edict_t *ent, vec3_t oldvel)
{
	...
	for (i=0 ; i<8 ; i++)
	{
		...
		if (dr_fabs(oldorg[1] - ent->fields.server->origin[1]) > 4
		 || dr_fabs(oldorg[0] - ent->fields.server->origin[0]) > 4)
		{
...
void SV_WalkMove (prvm_edict_t *ent)
{
	...
	if (clip & 2)
	{
		...
		if (dr_fabs(start_velocity[0]) < 0.03125 && dr_fabs(start_velocity[1]) < 0.03125)
			return;
		...
		if (clip
		 && dr_fabs(originalmove_origin[1] - ent->fields.server->origin[1]) < 0.03125
		 && dr_fabs(originalmove_origin[0] - ent->fields.server->origin[0]) < 0.03125)
		{
			//Con_Printf("wall\n");
	...
	if (downtrace.fraction < 1 && downtrace.plane.normal[2] > 0.7)
	{
		...
		// up while already jumping (also known as the Quake2 2*jump bug)
#if 0
	...
	if (trace.fraction < 1)
	{
		...
		else if (ent->fields.server->movetype == MOVETYPE_BOUNCE)
		{
			...
			if (sv_gameplayfix_grenadebouncedownslopes.integer)
			{
				...
				if (trace.plane.normal[2] > 0.7 && dr_fabs(d) < 60)
				{
					ent->fields.server->flags = (int)ent->fields.server->flags | FL_ONGROUND;
					ent->fields.server->groundentity = PRVM_EDICT_TO_PROG(trace.ent);
					VectorClear (ent->fields.server->velocity);
