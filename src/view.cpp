...
// don't count small mouse motion
	if (cl.nodrift)
	{
		if ( dr_fabs(cl.cmd.forwardmove) < cl_forwardspeed.value)
			cl.driftmove = 0;
		else
	...
	if (cls.state == ca_connected && cls.signon == SIGNONS)
	{
		...
		else
		{
			...
			else
			{
				...
				{
					dr_real_t xyspeed, bob;

					xyspeed = dr_sqrt(cl.movement_velocity[0]*cl.movement_velocity[0] + cl.movement_velocity[1]*cl.movement_velocity[1]);
					if (cl_bob.value && cl_bobcycle.value)
					{
						...
						if (cycle < cl_bobup.value)
							cycle = dr_sin(M_PI * cycle / cl_bobup.value);
						else
							cycle = dr_sin(M_PI + M_PI * (cycle-cl_bobup.value)/(1.0 - cl_bobup.value));
					...
					if (cl_bob.value && cl_bobmodel.value)
					{
						...
						bob = bspeed * cl_bobmodel_side.value * cl_viewmodel_scale.value * dr_sin(s) * t;
						VectorMA (gunorg, bob, right, gunorg);
						bob = bspeed * cl_bobmodel_up.value * cl_viewmodel_scale.value * dr_cos(s * 2) * t;
			...
			if (v_idlescale.value)
				Matrix4x4_CreateFromQuakeEntity(&r_view.matrix, vieworg[0], vieworg[1], vieworg[2], viewangles[0] + v_idlescale.value * dr_sin(cl.time*v_ipitch_cycle.value) * v_ipitch_level.value, viewangles[1] + v_idlescale.value * dr_sin(cl.time*v_iyaw_cycle.value) * v_iyaw_level.value, viewangles[2] + v_idlescale.value * dr_sin(cl.time*v_iroll_cycle.value) * v_iroll_level.value, 1);
			else
				Matrix4x4_CreateFromQuakeEntity(&r_view.matrix, vieworg[0], vieworg[1], vieworg[2], viewangles[0], viewangles[1], viewangles[2] + v_idlescale.value * dr_sin(cl.time*v_iroll_cycle.value) * v_iroll_level.value, 1);
...
void V_CalcViewBlend(void)
{
	...
	if (cls.state == ca_connected && cls.signon == SIGNONS && gl_polyblend.value > 0)
	{
		...
		{
			r_refdef.frustumscale_x *= 1 - (((dr_sin(cl.time * 4.7) + 1) * 0.015) * r_waterwarp.value);
			r_refdef.frustumscale_y *= 1 - (((dr_sin(cl.time * 3.0) + 1) * 0.015) * r_waterwarp.value);
