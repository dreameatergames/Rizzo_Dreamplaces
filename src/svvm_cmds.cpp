...
// get the PVS for the entity
	...
	if (sv.worldmodel && sv.worldmodel->t.brush.FatPVS)
		checkpvsbytes = sv.worldmodel->t.brush.FatPVS(sv.worldmodel, org, 0, checkpvs, sizeof(checkpvs));
...
static void VM_SV_checkclient (void)
{
	...
	if (sv.worldmodel && checkpvsbytes && !sv.worldmodel->t.brush.BoxTouchingPVS(sv.worldmodel, checkpvs, view, view))
	{
		c_notvis++;
		VM_RETURN_EDICT(prog->edicts);
		return;
...
static void VM_SV_walkmove (void)
{
	...
	move[0] = dr_cos(yaw)*dist;
	move[1] = dr_sin(yaw)*dist;
	move[2] = 0;
...
static void VM_SV_getlight (void)
{
	...
	if (sv.worldmodel && sv.worldmodel->t.brush.LightPoint)
		sv.worldmodel->t.brush.LightPoint(sv.worldmodel, p, ambientcolor, diffusecolor, diffusenormal);
	...
	*out = identitymatrix; // warnings and errors return identical matrix
	...
	if ((val = PRVM_EDICTFIELDVALUE(ent, prog->fieldoffsets.viewmodelforclient)) && val->edict)
	{
		...
		if (ent->fields.server->health > 0 && cl_bob.value && cl_bobcycle.value)
		{
			...
			bob = dr_sqrt(ent->fields.server->velocity[0]*ent->fields.server->velocity[0] + ent->fields.server->velocity[1]*ent->fields.server->velocity[1])*cl_bob.value;
...
VM_pow,							// #97 float(float f, float f) dr_pow (DP_QC_SINCOSSQRTPOW)
