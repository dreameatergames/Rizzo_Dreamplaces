...
static void SVBSP_PlaneFromPoints(dr_real_t *plane4f, const dr_real_t *p1, const dr_real_t *p2, const dr_real_t *p3)
{
	dr_real_t ilength;
...
void SVBSP_Init(svbsp_t *b, const dr_real_t *origin, int maxnodes, svbsp_node_t *nodes)
{
	memset(b, 0, sizeof(*b));
	b->origin[0] = origin[0];
	b->origin[1] = origin[1];
...
static void SVBSP_InsertOccluderPolygonNodes(svbsp_t *b, int *parentnodenumpointer, int parentnodenum, int numpoints, const dr_real_t *points, void (*fragmentcallback)(void *fragmentcallback_pointer1, int fragmentcallback_number1, svbsp_t *b, int numpoints, const dr_real_t *points), void *fragmentcallback_pointer1, int fragmentcallback_number1)
{
	...
	for (i = 0, p = numpoints - 1;i < numpoints;p = i, i++)
	{
		...
		{
			dr_real_t *parentnodeplane = b->nodes[j].plane;
			if (dr_fabs(SVBSP_DotProduct(b->origin     , parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON
			 && dr_fabs(SVBSP_DotProduct(points + p * 3, parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON
			 && dr_fabs(SVBSP_DotProduct(points + i * 3, parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON)
				break;
		}
		if (j >= 0)
		...
		// wrong side
		// (in this way this code treats all polygons as dr_real_t sided)
		//
		...
		{
			dr_real_t d = SVBSP_DotProduct(points + j * 3, node->plane) - node->plane[3];
	...
	{
		dr_real_t *parentnodeplane = b->nodes[j].plane;
		if (dr_fabs(SVBSP_DotProduct(points    , parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON
		 && dr_fabs(SVBSP_DotProduct(points + 3, parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON
		 && dr_fabs(SVBSP_DotProduct(points + 6, parentnodeplane) - parentnodeplane[3]) < SVBSP_CLIP_EPSILON)
			break;
	}
	if (j < 0)
#endif
...
static int SVBSP_AddPolygonNode(svbsp_t *b, int *parentnodenumpointer, int parentnodenum, int numpoints, const dr_real_t *points, int insertoccluder, void (*fragmentcallback)(void *fragmentcallback_pointer1, int fragmentcallback_number1, svbsp_t *b, int numpoints, const dr_real_t *points), void *fragmentcallback_pointer1, int fragmentcallback_number1)
{
	int i;
	int frontnumpoints, backnumpoints;
	dr_real_t frontpoints[MAX_SVBSP_POLYGONPOINTS * 3], backpoints[MAX_SVBSP_POLYGONPOINTS * 3];
	if (numpoints < 3)
		return 0;
...
int SVBSP_AddPolygon(svbsp_t *b, int numpoints, const dr_real_t *points, int insertoccluder, void (*fragmentcallback)(void *fragmentcallback_pointer1, int fragmentcallback_number1, svbsp_t *b, int numpoints, const dr_real_t *points), void *fragmentcallback_pointer1, int fragmentcallback_number1)
{
	int i;
	int nodenum;
