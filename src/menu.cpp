...
static void M_Options_PrintCommand(const char *s, int enabled)
{
	...
	{
		if (optnum == optcursor)
			DrawQ_Pic(menu_x + 48, menu_y + opty, NULL, 320, 8, optnum == optcursor ? (0.5 + 0.2 * dr_sin(realtime * M_PI)) : 0, 0, 0, 0.5, 0);
...
void M_Menu_Video_f (void)
{
	...
	for (i = 1;i < VID_RES_COUNT;i++)
	{
		...
		if (dr_fabs(video_resolutions[i].width - vid.width) > dr_fabs(video_resolutions[video_resolution].width - vid.width))
			continue;
		...
		if (video_resolutions[i].width == vid.width && video_resolutions[video_resolution].width == vid.width)
		{
			...
			if (dr_fabs(video_resolutions[i].height - vid.height) > dr_fabs(video_resolutions[video_resolution].height - vid.height))
				continue;
			...
			if (video_resolutions[i].height == vid.height && video_resolutions[video_resolution].height == vid.height)
			{
				...
				if (dr_fabs(video_resolutions[i].pixelheight - vid_pixelheight.value) > dr_fabs(video_resolutions[video_resolution].pixelheight - vid_pixelheight.value))
					continue;
...
static void M_ServerList_Draw (void)
{
	...
	if (end > start)
	{
		for (n = start;n < end;n++)
		{
			DrawQ_Pic(menu_x, menu_y + y, NULL, 640, 16, n == slist_cursor ? (0.5 + 0.2 * dr_sin(realtime * M_PI)) : 0, 0, 0, 0.5, 0);
