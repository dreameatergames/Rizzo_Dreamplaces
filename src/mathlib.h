...
#define lhrandom(MIN,MAX) (((dr_real_t)rand() / RAND_MAX) * ((MAX)-(MIN)) + (MIN))

#define invpow(base,number) (dr_log(number) / dr_log(base))
...
#define VectorNormalize(v) {float ilength = (float) dr_sqrt(DotProduct(v,v));if (ilength) ilength = 1.0f / ilength;v[0] *= ilength;v[1] *= ilength;v[2] *= ilength;}
#define VectorNormalize2(v,dest) {float ilength = (float) dr_sqrt(DotProduct(v,v));if (ilength) ilength = 1.0f / ilength;dest[0] = v[0] * ilength;dest[1] = v[1] * ilength;dest[2] = v[2] * ilength;}
#ifdef NO_DOUBLE
#define VectorNormalizeDouble(v) VectorNormalize(v)
#else
#define VectorNormalizeDouble(v) {dr_real_t ilength = dr_sqrt(DotProduct(v,v));if (ilength) ilength = 1.0 / ilength;v[0] *= ilength;v[1] *= ilength;v[2] *= ilength;}
#endif
#define VectorDistance2(a, b) (((a)[0] - (b)[0]) * ((a)[0] - (b)[0]) + ((a)[1] - (b)[1]) * ((a)[1] - (b)[1]) + ((a)[2] - (b)[2]) * ((a)[2] - (b)[2]))
#define VectorDistance(a, b) (dr_sqrt(VectorDistance2(a,b)))
#define VectorLength(a) (dr_sqrt(DotProduct(a, a)))
...
#define VectorReflect(a,r,b,c) do{dr_real_t d;d = DotProduct((a), (b)) * -(1.0 + (r));VectorMA((a), (d), (b), (c));}while(0)
...
#define qlen(a) ((float) dr_sqrt((a)[0]*(a)[0]+(a)[1]*(a)[1]+(a)[2]*(a)[2]+(a)[3]*(a)[3]))
...
#define qdist(a,b) ((float) dr_sqrt(((b)[0]-(a)[0])*((b)[0]-(a)[0])+((b)[1]-(a)[1])*((b)[1]-(a)[1])+((b)[2]-(a)[2])*((b)[2]-(a)[2])+((b)[3]-(a)[3])*((b)[3]-(a)[3])))
...
void VectorVectorsDouble(const dr_real_t *forward, dr_real_t *right, dr_real_t *up);
