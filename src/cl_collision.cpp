...
void CL_FindNonSolidLocation(const vec3_t in, vec3_t out, vec_t radius)
{
	...
	if (cl.worldmodel && cl.worldmodel->t.brush.FindNonSolidLocation)
		cl.worldmodel->t.brush.FindNonSolidLocation(cl.worldmodel, in, out, radius);
}
...
trace_t CL_Move(const vec3_t start, const vec3_t mins, const vec3_t maxs, const vec3_t end, int type, prvm_edict_t *passedict, int hitsupercontentsmask, qboolean hitnetworkbrushmodels, qboolean hitnetworkplayers, int *hitnetworkentity, qboolean hitcsqcentities)
{
	...
	if (cl.worldmodel && cl.worldmodel->t.brush.RoundUpToHullSize)
		cl.worldmodel->t.brush.RoundUpToHullSize(cl.worldmodel, clipmins, clipmaxs, hullmins, hullmaxs);
	else
	{
